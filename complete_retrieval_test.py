...
def test_query_with_agent(agent, query: str, mode: str, timeout: int = 120) -> Dict[str, Any]:
...
def run_complete_test():
    ...
    for i, query in enumerate(TEST_QUERIES, 1):
        ...
        for mode in RETRIEVAL_MODES:
            ...
            # Log result
            if result['success']:
                logger.info(f"✓ {mode}: {result['execution_time']:.1f}s, Rows: {result['row_count']}")
...
def generate_summary(results: List[Dict]):
    ...
    for mode in RETRIEVAL_MODES:
        ...
        print(f"\n{mode.upper()} Mode:")
        print(f"  Success Rate: {successful}/{len(mode_results)} ({successful/len(mode_results)*100:.1f}%)")
    ...
    for query in TEST_QUERIES:
        ...
        if successful:
            ...
            if any("Marienstr" in query for query in [query]):
                correct_answers = [r for r in successful if "Petra Nabakowski" in str(r.get('answer', ''))]
                print(f"  Correct answers: {len(correct_answers)}/{len(successful)} modes")
