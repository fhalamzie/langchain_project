...
class WincasaDataSampler:
    ...
    def __init__(self, db_path: str = "WINCASA2022.FDB"):
        ...
        self.priority_tables = {
            "core_entities": [
                ...
                "BEWADR", "EIGADR", "KONTEN"
            ],
            "financial": [
                "BUCHUNG", "SOLLSTELLUNG", "BANKEN", "VORAUSZ"
            ],
            "management": [
                "VERWALTER", "SEVERTRAG", "NACHWEIS", "BESCHLUSS"
            ],
            "operational": [
                "HDBUCH", "ZTERMIN", "VEREIG", "LIEFERAN"
            ]
        }
    ...
    def get_table_sample(self, table_name: str, limit: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if not columns_result:
                sample_info["errors"].append(f"Table {table_name} not found or no columns")
                return sample_info
            ...
            # Analyze patterns
            sample_info["patterns"] = self._analyze_patterns(sample_info["sample_data"], column_names)
    ...
    def _analyze_patterns(self, sample_data: List[Dict], columns: List[str]) -> Dict[str, Any]:
        ...
        for col in columns:
            ...
            col_patterns = {
                ...
                "sample_values": list(set(str(v) for v in col_values[:10]))  # First 10 unique values
            }
            ...
            for category, tables in self.priority_tables.items():
                ...
                for table_name in tables:
                    ...
                    if sample["errors"]:
                        ...
                        print(f"    ⚠️  {len(sample['errors'])} errors: {sample['errors'][0][:100]}...")
                    else:
                        all_samples["summary"]["successful_samples"] += 1
                        all_samples["summary"]["total_rows_sampled"] += sample["sample_size"]
    ...
    def save_samples_to_file(self, samples: Dict[str, Any], output_path: str = "output/data_samples.json"):
    ...
    def generate_context_summary(self, samples: Dict[str, Any]) -> str:
        ...
        for category, category_samples in samples["samples"].items():
            ...
            for table_name, sample in category_samples.items():
                ...
                important_cols = ["ONR", "BEWNR", "EIGNR", "KNR", "NAME", "VORNAME", "BSTR", "BPLZORT"]
                for col in important_cols:
                    if col in sample["patterns"] and sample["patterns"][col]["sample_values"]:
...
def main():
    ...
    print("\n📝 Context summary:")
    print(context_summary[:1000] + "..." if len(context_summary) > 1000 else context_summary)
