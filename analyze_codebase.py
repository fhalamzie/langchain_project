...
def analyze_file_imports(file_path: str) -> Dict[str, Any]:
    ...
    if len(imports) > 1:
        ...
        stdlib_modules = {'sys', 'os', 'json', 'time', 'datetime', 'pathlib', 'typing', 're', 'tempfile', 'subprocess'}
        ...
        if stdlib_imports and third_party_imports:
            ...
            if max(stdlib_lines) > min(third_party_lines):
                issues.append("Imports not properly grouped (stdlib mixed with third-party)")
    ...
    for i, line in enumerate(lines, 1):
        ...
        if 'eval(' in line or 'exec(' in line:
            issues.append({
                ...
                "code": line.strip()[:60] + "..." if len(line.strip()) > 60 else line.strip()
            })
        
        # Check for hardcoded passwords/secrets
        if any(keyword in line.lower() for keyword in ['password', 'secret', 'key', 'token']) and '=' in line:
            if not any(safe in line.lower() for safe in ['os.environ', 'getenv', 'config']):
                issues.append({
                    "line": i,
                    "severity": "MEDIUM", 
...
def main():
    ...
    if results['total_issues'] == 0:
        ...
        if results['black_issues'] > 0:
            print(f"  🎨 Run 'black .' to fix {results['black_issues']} formatting issues")
        
        if results['isort_issues'] > 0:
            print(f"  📦 Run 'isort .' to fix {results['isort_issues']} import organization issues")
        
        if results['security_issues'] > 0:
            print(f"  🛡️ Review and fix {results['security_issues']} security issues manually")
