...
def test_all_modes():
    ...
    try:
                        ...
                        for query in test_queries:
                            ...
                            try:
                                ...
                                if docs and len(docs) > 0:
                                    ...
                                    logger.info(f"✅ Query succeeded in {execution_time:.2f}s")
                                else:
                                    result = {
                                        "query": query,
                                        "success": False,
        ...
        for mode, mode_results in results.items():
            ...
            print(f"{status} {mode}: {successful}/{total} queries succeeded ({success_rate:.0f}%)")
            
            if mode_results and "execution_time" in mode_results[0]:
                avg_time = sum(r.get("execution_time", 0) for r in mode_results) / len(mode_results)
