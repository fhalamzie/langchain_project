...
class EnhancedMultiStageRetriever(BaseDocumentationRetriever):
    ...
    def _initialize_specialized_retrievers(self):
        ...
        if self.relationship_docs:
            self.relationship_store = FAISS.from_documents(self.relationship_docs, embeddings)
    ...
    def _create_enhanced_schema_docs(self):
        ...
        overview_doc = Document(
            ...
            metadata={'source': 'compiled_knowledge', 'type': 'overview', 'priority': 10}
        )
        ...
        for from_table, to_tables in rel_matrix.items():
            rel_content = f"Table {from_table} has relationships to: {', '.join(to_tables)}"
            rel_doc = Document(
                page_content=rel_content,
                metadata={'source': 'relationship_matrix', 'type': 'relationship', 'priority': 8}
            )
...
"""
                enhanced_doc = Document(
                    ...
                    metadata={'source': 'enhanced_schema', 'table': table_name, 'priority': 9}
                )
    ...
    def get_relevant_documents(self, query: str) -> List[Document]:
        ...
        try:
            ...
            logger.info(f"Stage 1 retrieved {len(stage1_docs)} docs in {time.time() - stage1_start:.2f}s")
            
            # Stage 2: Query-Specific Documentation
            stage2_start = time.time()
            stage2_docs = self._stage2_query_specific(query)
            all_docs.extend(stage2_docs)
            logger.info(f"Stage 2 retrieved {len(stage2_docs)} docs in {time.time() - stage2_start:.2f}s")
            
            # Stage 3: Historical Query Patterns
            stage3_start = time.time()
            stage3_docs = self._stage3_historical_patterns(query)
            all_docs.extend(stage3_docs)
            logger.info(f"Stage 3 retrieved {len(stage3_docs)} docs in {time.time() - stage3_start:.2f}s")
    ...
    def _stage2_query_specific(self, query: str) -> List[Document]:
        ...
        # Financial queries
        if any(term in query_lower for term in ['zahlung', 'payment', 'saldo', 'balance', 'konto', 'account']):
            if self.procedure_docs:
                proc_store = FAISS.from_documents(self.procedure_docs, 
                                                 OpenAIEmbeddings(openai_api_key=self.openai_api_key))
                financial_procs = proc_store.similarity_search("SALDO ZAHLUNG KONTO financial", k=3)
                docs.extend(financial_procs)
        
        # Property queries
        elif any(term in query_lower for term in ['objekt', 'property', 'wohnung', 'apartment', 'gebäude', 'building']):
            if self.schema_store:
                property_docs = self.schema_store.similarity_search("OBJEKTE WOHNUNG property management", k=3)
                docs.extend(property_docs)
        
        # Person queries
        elif any(term in query_lower for term in ['eigentümer', 'owner', 'bewohner', 'tenant', 'mieter']):
            if self.schema_store:
                person_docs = self.schema_store.similarity_search("EIGENTUEMER BEWOHNER person contact", k=3)
                docs.extend(person_docs)
        
        # Get examples if available
        if self.example_docs:
            example_store = FAISS.from_documents(self.example_docs,
                                               OpenAIEmbeddings(openai_api_key=self.openai_api_key))
...
"""
            ...
            if any(table.lower() in query.lower() for _ in [table]):
                for pattern in patterns[:2]:  # Limit patterns per table
                    pattern_doc = Document(
                        ...
                        metadata={'source': 'query_patterns', 'table': table, 'stage': 3}
                    )
    ...
    def _extract_table_names(self, query: str) -> List[str]:
        ...
        all_tables.extend([t['table'] for t in self.knowledge_base.get('top_20_tables', [])])
    ...
    def get_context_with_compression(self, query: str, llm: Optional[Any] = None) -> List[Document]:
        """
...
class EnhancedFaissRetriever(FaissDocumentationRetriever):
    ...
    def _get_relevant_documents(self, query: str) -> List[Document]:
        ...
        try:
            ...
            has_overview = any(d.metadata.get('source') == 'database_overview' for d in docs)
            if not has_overview and hasattr(self, '_enhanced_docs'):
                overview_doc = next((d for d in self._enhanced_docs 
                                   if d.metadata.get('source') == 'database_overview'), None)
                if overview_doc:
