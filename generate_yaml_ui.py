...
TOKEN_MARGIN = 5000
llm = ChatOpenAI(model="gpt-4-1106-preview", temperature=0, openai_api_key=openai_api_key)
...
def merge_yaml_outputs(yaml_list):
    ...
    for field in ["description", "business_context", "internal_conventions", "machine_constraints"]:
    ...
    merged["table_name"] = merged.get("table_name", yaml_list[0].get("table_name", "MERGED_PROCEDURE"))
    return merged
...
def safe_llm_call(prompt, item_name, item_type="table"):
    try:
        ...
        log_message(f"Prompt für {item_name}: ca. {token_estimate} Tokens (Schätzung)", "info", save_status=False)
        
        if token_estimate > (MAX_MODEL_TOKENS - TOKEN_MARGIN):
            log_message(f"⏩ {item_type.capitalize()} '{item_name}' übersprungen (Prompt zu groß: {int(token_estimate)} Tokens)", "warning")
            print(f"⏩ SKIP {item_name} wegen zu großem Prompt ({int(token_estimate)} tokens geschätzt!)")
        ...
        try:
            ...
            if isinstance(prompt, list) and isinstance(prompt[0], dict):
                ...
                if prompt_size > max_safe_size:
                    log_message(f"⏩ {item_type.capitalize()} '{item_name}' übersprungen (Prompt-Byte-Größe zu groß: {prompt_size} Bytes)", "warning")
                    log_skipped_item(item_name, item_type)
                    return None
        except Exception as e:
        ...
        if isinstance(prompt, list) and isinstance(prompt[0], dict) and "role" not in prompt[0]:
...
def translate_to_german(text):
    prompt = [{"role": "user", "content": f"Übersetze folgenden Text ins Deutsche und korrigiere alle eventuellen Fehler:\n\n{text}"}]
            ...
            for i, line in enumerate(lines):
                if i > 0 and line.strip() and ":" not in line and not line.strip().startswith("-") and not line.strip().startswith("#"):
                    ...
                    if ":" in line:
                        ...
                        if match:
                            indentation = match.group(1) + "  "  # Einrückung plus 2 Leerzeichen
...
"""
        ...
        if valid:
            ...
            return False, f"Reparatur für {table_name} hat nicht zu gültigem YAML geführt."
    except Exception as e:
...
def get_db_connection():
    ...
    try:
        ...
        if not os.path.exists(DB_PATH):
            log_message(f"Datenbank-Datei '{DB_PATH}' nicht gefunden. Falls Sie die Datenbank haben, stellen Sie sicher, dass sie im richtigen Pfad liegt.", "warning")
            yield None
            return
            
        conn = fdb.connect(dsn=DB_PATH, user=DB_USER, password=DB_PASSWORD, charset="WIN1252")
        yield conn
    except Exception as e:
        log_message(f"Fehler bei der Datenbankverbindung: {e}", "error")
        log_message("Das Skript wird ohne Datenbankverbindung fortgesetzt. Einige Funktionen könnten eingeschränkt sein.", "warning")
        yield None
    finally:
        if conn:
            conn.close()
...
def get_all_tables():
    try:
        with get_db_connection() as conn:
            if conn is None:
                log_message("Keine Datenbankverbindung verfügbar. Verwende Beispieltabellen.", "warning")
...
def get_table_columns(table_name: str) -> List[str]:
    with get_db_connection() as conn:
        ...
        try:
            ...
            log_message(f"Fehler beim Abrufen der Spalten für {table_name}: {e}", "warning")
            return []
    ...
    try:
        ...
        if sample_size is None:
            if col_count > 150:
                ...
                max_sample_size = min(DEFAULT_MAX_SAMPLE_SIZE, max(5, int(400 / max(1, col_count))))
        else:
        ...
        with get_db_connection() as conn:
            ...
            try:
                ...
                converted_rows = []
                for row in rows:
                    converted_row = [convert_win1252_to_utf8(item) if isinstance(item, bytes) else item for item in row]
                    converted_rows.append(converted_row)
                return pd.DataFrame(converted_rows, columns=columns)
            except Exception as e:
                log_message(f"Fehler beim Abrufen von Beispieldaten aus {table}: {e}", "warning")
                return pd.DataFrame()
    except Exception as e:
...
def get_column_metadata(table_name: str) -> Dict[str, Dict[str, Any]]:
    try:
        with get_db_connection() as conn:
            ...
            """, (table_name,))
            type_map = {7: "SMALLINT", 8: "INTEGER", 10: "FLOAT", 12: "DATE", 13: "TIME",
                        14: "CHAR", 16: "BIGINT", 27: "DOUBLE PRECISION", 35: "TIMESTAMP", 37: "VARCHAR", 261: "BLOB"}
            columns = {}
            for name, type_id, length, precision, scale, not_null, default, desc in cursor.fetchall():
                name = name.strip()
                data_type = type_map.get(type_id, f"UNKNOWN({type_id})")
                if data_type in ("SMALLINT", "INTEGER", "BIGINT") and scale and scale < 0:
        ...
        log_message(f"Fehler beim Abrufen der Spaltenmetadaten für {table_name}: {e}", "error")
        return {}
        ...
        with get_db_connection() as conn:
            ...
            for f1, tab2, f2 in cur.fetchall():
                constraints["foreign_keys"].append({
                    "field": f1.strip(), "references_table": tab2.strip(), "references_field": f2.strip()
                })
            cur.execute("""
            ...
            for fname, default, notnull, check in cur.fetchall():
                if default:
                    constraints["defaults"][fname.strip()] = convert_win1252_to_utf8(default).strip()
                if check:
                    constraints["checks"].append({"field": fname.strip(), "rule": convert_win1252_to_utf8(check).strip()})
            return constraints
    except Exception as e:
        log_message(f"Fehler beim Abrufen der Constraints für {table_name}: {e}", "error")
        return {"primary_key": [], "unique": [], "foreign_keys": [], "checks": [], "defaults": {}}
...
def get_table_relations(table_name: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    try:
        ...
        log_message(f"Fehler beim Abfragen von Beziehungen für {table_name}: {e}", "warning")
    return relations
    ...
    try:
        with get_db_connection() as conn:
            ...
            for col_name, col_info in metadata.items():
                ...
                column_sql.append(f"    {col_name} {data_type} {nullable} {default}".strip())
            ...
            for fk in constraints['foreign_keys']:
                column_sql.append(f"    FOREIGN KEY ({fk['field']}) REFERENCES {fk['references_table']}({fk['references_field']})")
...
def get_procedure_ddl(proc_name: str) -> str:
    try:
        ...
        log_message(f"Fehler beim Abrufen des DDL für Prozedur {proc_name}: {e}", "warning")
        return f"-- Fehler beim Generieren des Prozedur-DDL für {proc_name}"

# ==== SEMANTIK-FUNKTIONEN
SENSITIVE_COLUMNS = {
    "name": ["name", "vorname", "nachname", "kunde", "mieter", "person", "ansprechpartner", "kontakt"],
    ...
    "id": ["personalausweis", "ausweis", "pass", "reisepass", "id", "kundennr", "mieternr", "vertragsnr"],
    "account": ["konto", "iban", "kontonr", "blz", "bank"],
    "finance": ["gehalt", "einkommen", "miete", "betrag", "summe", "preis", "zahlung"]
}
SPECIAL_KUERZEL_FIELDS = {"EIGADR": {"ENOTIZ": "owner_note"}, "OBJEKTE": {"OBEZ": "street_code"}}

def identify_column_type(column_name: str, sample_data: pd.Series = None, table_name: str = "") -> str:
    """
...
def generate_street_code_example(street_name: str = "", house_number: str = "") -> str:
    if not street_name:
        street_examples = ["Marienstraße", "Hochfelder", "Lindenallee", "Hauptstraße", "Bergweg"]
    ...
    street_clean = street_name.upper().replace("Ä", "AE").replace("Ö", "OE").replace("Ü", "UE").replace("ß", "SS")
...
def generate_sev_owner_note(first_name: str = None, last_name: str = None) -> str:
    ...
    last_name_clean = last_name.upper().replace("Ä", "AE").replace("Ö", "OE").replace("Ü", "UE").replace("ß", "SS")
    ...
    try:
        ...
        if semantic_type == "name":
            ...
            try:
                ...
                return f"{orig_val + random.uniform(-variance, variance):.2f} €".replace('.', ',')
            except:
...
def profile_data(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    if df.empty:
        return {}
    return {column: {"data_type": str(df[column].dtype),
...
def is_temporary_trivial_table(table_name, df, column_metadata, relations):
    if any(pat in table_name.lower() for pat in ["temp", "tmp", "cache", "backup", "hist", "log", "vorl"]):
        return True
    ...
    if df is not None and not df.empty:
        ...
        for col in df.columns:
            ...
            if all(str(x).strip() in ["", "0", "0.0", "None", "NaN"] for x in series):
                ...
                trivial = False
                break
        if trivial and (not relations["foreign_keys"] and not relations["referenced_by"]):
            return True
            
    if column_metadata and all(meta.get("is_nullable", True) for meta in column_metadata.values()) and (not relations["foreign_keys"] and not relations["referenced_by"]):
        return True
...
def determine_table_priority(table_name, column_metadata, relations):
    ...
    if "objekt" in normalized_name or "mieter" in normalized_name or "vertrag" in normalized_name:
        return "high"
...
def typical_use_cases(table_name, columns, relations):
    ...
    if "objekt" in normalized or "immobilie" in normalized:
        result += ["Verknüpfung von Bewohnern und Mietverträgen mit Immobilien.",
                   "Abfrage von Ausstattung und Zustand einzelner Objekte.",
                   "Abfrage, in welcher Straße sich ein Objekt befindet."]
    if "mieter" in normalized or "kunde" in normalized or "person" in normalized or "bewohner" in normalized:
        result += ["Abfrage und Verwaltung persönlicher Kontaktdaten von Mietern.",
                   "Zuordnung von Mietern zu Wohnungen/Einheiten.",
                   "Nachverfolgung der Mietdauer eines Bewohners."]
    if "eigent" in normalized:
        result += ["Verwaltung von Eigentümerinformationen und -kontaktdaten.",
    ...
    if "vertrag" in normalized:
        result += ["Verwaltung von Start-, Endtermin und Konditionen des Mietvertrags.",
...
def typical_queries(table_name, columns, relations):
    ...
    if table_name.upper() == "EIGADR" or "enotiz" in column_names:
        ...
        queries.append("Ist der Eigentümer mit Kürzel FHUBER ein SEV- oder WEG-Eigentümer?")
    ...
    if "adresse" in column_names or "straße" in column_names or "strasse" in column_names:
        queries.append("Wer wohnt aktuell in der Marienstraße 26?")
        queries.append("Wie hoch ist die durchschnittliche Miete in der Hochfelder Straße?")
    if not queries:
...
def process_table(table_name, phase="phase1", translate_english=False):
    try:
        ...
        temp_tote = is_temporary_trivial_table(table_name, df if not is_empty else None, column_metadata, relations)
        priority = "low" if temp_tote else determine_table_priority(table_name, column_metadata, relations)
        ...
        if not is_empty:
            ...
            for column in df.columns:
                ...
                if semantic_type != "regular":
                    anonymized_columns[column] = df[column].apply(lambda x: generate_anonymized_example(x, semantic_type))
                else:
                    anonymized_columns[column] = df[column]
            anonymized_df = pd.DataFrame(anonymized_columns)
        else:
            semantic_types = {col: identify_column_type(col, None, table_name) for col in column_metadata.keys()}
        ...
        for col_name, meta in column_metadata.items():
            ...
            if table_name.upper() in SPECIAL_KUERZEL_FIELDS:
                mapped_sem_type = SPECIAL_KUERZEL_FIELDS[table_name.upper()].get(col_name.upper())
            ...
            col_data = {
                ...
                **meta,
                ...
                "example_values": [str(row.get(col_name, "")) for row in sample[:3]] if not is_empty and sample else []
            }
        ...
        prompt = prompt_template.format_messages(
            ...
            ddl_constraints=json.dumps(ddl_constraints, ensure_ascii=False, default=str),
...
def get_procedure_info(procedure_name: str) -> Dict:
    try:
        ...
        log_message(f"Fehler beim Abrufen der Prozedurinfos für {procedure_name}: {e}", "error")
        return {}
...
def process_procedure(proc_name, phase="phase1"):
    try:
        ...
        if len(columns) <= max_cols_per_prompt:
            ...
            log_message(f"Prozedur {proc_info['name']} hat {len(columns)} Parameter, splitte auf Blöcke.", "info")
        ...
        for chunk_idx, cols_chunk in enumerate(blocks):
            prompt = prompt_template.format_messages(
                table_name=proc_info["name"] + (f"_block{chunk_idx+1}" if len(blocks) > 1 else ""),
            ...
            response = safe_llm_call(prompt, f"{proc_info['name']} Block {chunk_idx+1}", item_type="procedure")
            
            if response is None:
                print(f"⏩ SKIP Prozedur {proc_info['name']} Block {chunk_idx+1} (Prompt zu groß)")
                continue
            ...
            if not is_valid:
                log_message(f"YAML ungültig für Prozedur {proc_info['name']} Block {chunk_idx+1}: {cleaned_yaml}", "error")
                continue
        ...
        if not yaml_outputs:
            return False, f"⚠️ Keine gültigen Chunks für Prozedur {proc_info['name']}, übersprungen."
        ...
        return True, f"✅ YAML für Prozedur {proc_info['name']} generiert (Chunking/merged)"
    except Exception as e:
...
def generate_yaml_batch(existing_yamls={}, force_regenerate=False, phase="phase1", table_subset=None, translate_english=False):
    try:
        ...
        for table in tables_to_process:
            ...
            already_in_phase2 = (
                phase == "phase2" and
                table in status.generated_details and
                status.generated_details[table] == "phase2"
            )
            
            if already_in_phase2:
                completed += 1
                log_message(f"⏭️ Überspringe Tabelle bereits in Phase 2: {table} ({completed}/{tables_count})")
                continue
                
            if yaml_path.exists() and not force_regenerate and table not in existing_yamls:
                completed += 1
                log_message(f"⏭️ Überspringe existierende Tabelle: {table} ({completed}/{tables_count})")
                continue
                
            success_flag, message = process_table(table, phase=phase, translate_english=translate_english)
            log_message(message)
            ...
            print(f"Fortschritt: {completed}/{tables_count} Tabellen ({success} erfolgreich, {skipped} übersprungen)")
        
        log_message(f"Batch abgeschlossen: {success} generiert, {skipped} übersprungen", "info")
...
def generate_all_procedure_yaml(phase="phase1"):
    try:
        ...
        for i, proc_name in enumerate(procs, 1):
            ...
            already_in_phase2 = (
                phase == "phase2" and
                proc_key in status.generated_details and
                status.generated_details[proc_key] == "phase2"
            )
            
            if already_in_phase2:
                log_message(f"⏭️ Überspringe Prozedur bereits in Phase 2: {proc_name} ({i}/{total})")
                continue
                
            if yaml_path.exists():
                log_message(f"⏭️ Überspringe existierende Prozedur: {proc_name} ({i}/{total})")
                continue
        ...
        log_message(f"Prozedur-Batch abgeschlossen: {success} generiert, {failures} übersprungen/fehlgeschlagen")
    ...
    try:
        ...
        with open(index_file, "w", encoding="utf-8") as f:
                ...
                if "_procedure" not in yaml_file.stem:
                    try:
                        with open(yaml_file, "r", encoding="utf-8") as yf:
                            ...
                            log_message(f"Tabelle {yaml_file.stem} erfolgreich verarbeitet", "info", save_status=False)
                    except Exception as e:
                        log_message(f"Fehler beim Verarbeiten von {yaml_file.stem}: {str(e)}", "warning", save_status=False)
                        yamls.append({
                            "name": yaml_file.stem,
                            "priority": "unknown",
                            "is_empty": False,
            ...
            for item in yamls:
                ...
                f.write(f"- {priority_emoji} [{item['name']}]({item['name']}.md){empty_flag} - {item['description']}\n")
            ...
            for yaml_file in yaml_output_dir.glob("*_procedure.yaml"):
                ...
                try:
                    with open(yaml_file, "r", encoding="utf-8") as yf:
                        ...
                        log_message(f"Prozedur {name} erfolgreich verarbeitet", "info", save_status=False)
                except Exception as e:
                    log_message(f"Fehler beim Verarbeiten von Prozedur {name}: {str(e)}", "warning", save_status=False)
                    procedures.append({
                        "name": name,
                        "description": f"Prozedur {name}"
                    })
            ...
            for item in procedures:
                f.write(f"- [{item['name']}]({item['name']}_proc.md) - {item['description']}\n")
                    ...
                    if "_procedure" in yaml_file.stem:
                        ...
                        with open(md_file, "w", encoding="utf-8") as f:
                            ...
                            if "columns" in data and data["columns"]:
                                ...
                                for col in data["columns"]:
                                    ...
                                    direction = "Eingang" if "IN-" in desc else "Ausgang" if "OUT-" in desc else "-"
                                    f.write(f"| {col.get('name', '')} | {col.get('type', '')} | {direction} | {desc} |\n")
                            else:
                                f.write("_Keine Parameter dokumentiert_\n")
                            
                            if "business_examples" in data and data["business_examples"]:
                        ...
                        with open(md_file, "w", encoding="utf-8") as f:
                            ...
                            if data.get("internal_conventions"):
                                f.write(f"## Interne Konventionen\n\n{data.get('internal_conventions', '')}\n\n")
                            ...
                            if "columns" in data and data["columns"]:
                                ...
                                f.write("|------|-----|----------|-------------|\n")
                                for col in data["columns"]:
                                    nullable = "Ja" if col.get("nullable", True) else "Nein"
                                    desc = col.get("description", "").replace("\n", " ")
                                    f.write(f"| {col.get('name', '')} | {col.get('type', '')} | {nullable} | {desc} |\n")
                            else:
                            ...
                            if "relations" in data and data["relations"]:
                                ...
                                if isinstance(data["relations"], dict):
                                    # Write foreign keys
                                    if "foreign_keys" in data["relations"] and data["relations"]["foreign_keys"]:
                                        ...
                                        for fk in data["relations"]["foreign_keys"]:
                                            if isinstance(fk, dict):
                                                # Dictionary-Format
                                                f.write(f"| {fk.get('field', '')} | {fk.get('references_table', '')}({fk.get('references_field', '')}) |\n")
                                                ...
                                                if len(parts) == 2:
                                                    ...
                                                    if match:
                                                        ...
                                                        f.write(f"| {from_field} | {to_table}({to_field}) |\n")
                                                    else:
                                                        f.write(f"| {from_field} | {to_part} |\n")
                                    
                                    # Write references
                                    if "referenced_by" in data["relations"] and data["relations"]["referenced_by"]:
                                        f.write("\n### Referenziert von\n\n")
                                        ...
                                        for ref in data["relations"]["referenced_by"]:
                                            if isinstance(ref, dict):
                                                # Dictionary-Format
                                                f.write(f"| {ref.get('table', '')} | {ref.get('field', '')} | {ref.get('via_field', '')} |\n")
                                                ...
                                                if len(parts) == 2:
                                                    ...
                                                    f.write(f"| {ref_table} | {ref_field} | - |\n")
                                                else:
                                                    f.write(f"| {ref} | - | - |\n")
                                else:
                                    # Simple list format
                                    f.write("\n".join([f"- {rel}" for rel in data["relations"]]))
        ...
        return True, f"Indexdatei erstellt mit {len(yamls)} Tabellen und {len(procedures)} Prozeduren."
    except Exception as e:
    ...
    try:
                            ...
                            for fk in data["relations"]["foreign_keys"]:
                                if isinstance(fk, dict):
                                    ...
                                    relationships.append({
                                        ...
                                        "label": f"{fk.get('field', '')} → {fk.get('references_field', '')}"
                                    })
                                elif isinstance(fk, str):
                                    # String-Format: "FIELD -> TABLE(FIELD)"
                                    parts = fk.split("->")
        ...
        with open(overview_file, "w", encoding="utf-8") as f:
            ...
            for t in sorted(high_priority, key=lambda x: x["name"]):
                f.write(f"- [{t['name']}]({t['name']}.md): {t['description'][:80]}...\n")
            ...
            f.write(f"- Leere Tabellen: {len([t for t in tables.values() if t['is_empty']])}\n")
            ...
            important_rels = [r for r in relationships if r["from_table"] in high_prio_names and r["to_table"] in high_prio_names]
            
            for rel in sorted(important_rels, key=lambda x: (x["from_table"], x["to_table"])):
                f.write(f"| [{rel['from_table']}]({rel['from_table']}.md) | [{rel['to_table']}]({rel['to_table']}.md) | {rel['from_column']} → {rel['to_column']} |\n")
        ...
        with open(dot_file, "w", encoding="utf-8") as f:
            ...
            for table_name, table in tables.items():
                color = "crimson" if table["priority"] == "high" else "darkorange" if table["priority"] == "medium" else "green3"
                style = "filled" if not table["is_empty"] else "filled,dashed"
                label = f"{table_name}"
                f.write(f"  \"{table_name}\" [label=\"{label}\", style=\"{style}\", fillcolor=\"{color}\", fontcolor=\"white\"];\n")
            
            # Kanten für Beziehungen
            for rel in relationships:
                f.write(f"  \"{rel['from_table']}\" -> \"{rel['to_table']}\" [label=\"{rel['from_column']} → {rel['to_column']}\"];\n")
        ...
        with open(high_prio_dot_file, "w", encoding="utf-8") as f:
            ...
            for table_name in high_prio_names:
                f.write(f"  \"{table_name}\" [label=\"{table_name}\", style=\"filled\", fillcolor=\"crimson\", fontcolor=\"white\"];\n")
            
            # Nur Beziehungen zwischen High-Prio-Tabellen
            for rel in important_rels:
                f.write(f"  \"{rel['from_table']}\" -> \"{rel['to_table']}\" [label=\"{rel['from_column']}\"];\n")
    ...
    try:
        ...
        with open(relations_file, "w", encoding="utf-8") as f:
            ...
            for table in all_tables:
                ...
                table_rel_count[table] = {"in": inbound, "out": outbound, "total": inbound + outbound}
            
            hub_tables = sorted(table_rel_count.items(), key=lambda x: x[1]["total"], reverse=True)[:15]
            
            f.write("## Top-Verbindungstabellen\n\n")
            f.write("| Tabelle | Eingehend | Ausgehend | Gesamt |\n")
            f.write("|---------|-----------|-----------|--------|\n")
            for table, counts in hub_tables:
                f.write(f"| [{table}]({table}.md) | {counts['in']} | {counts['out']} | {counts['total']} |\n")
        ...
        with open(clusters_file, "w", encoding="utf-8") as f:
            ...
            f.write("Diese Analyse gruppiert Tabellen basierend auf ihren Beziehungen zueinander.\n")
            f.write("Tabellen im selben Cluster sind durch Beziehungen (direkt oder indirekt) miteinander verbunden.\n\n")
            ...
            for i, cluster in enumerate(clusters, 1):
                ...
                if size > 20:
                    ...
                    cluster_hubs = sorted([(t, table_rel_count[t]["total"]) for t in cluster if t in table_rel_count], 
            ...
            for i, cluster in enumerate(clusters[:3], 1):  # Beschränke auf die 3 größten Cluster
                ...
                with open(cluster_dot_file, "w", encoding="utf-8") as df:
                    ...
                    df.write("  graph [rankdir=LR, fontname=\"Helvetica\", nodesep=0.8];\n")
                    ...
                    for table in cluster:
                        count = table_rel_count.get(table, {}).get("total", 0)
                        size = 1.0 + min(count / 10, 2.0)  # Größere Knoten für stärker vernetzte Tabellen
                        df.write(f"  \"{table}\" [label=\"{table}\", width={size}, height={size}, style=\"filled\", fillcolor=\"skyblue\"];\n")
                    
                    # Kanten
                    for rel in relationships:
                        if rel["from_table"] in cluster and rel["to_table"] in cluster:
                            df.write(f"  \"{rel['from_table']}\" -> \"{rel['to_table']}\" [label=\"{rel['from_column']}\"];\n")
            ...
            try:
                import subprocess
                for i in range(1, min(4, len(clusters) + 1)):  # For the 3 largest clusters or fewer if less exist
                    ...
                    if dot_file.exists():
                        ...
                        with open(clusters_file, "a", encoding="utf-8") as f:
                            ...
                            f.write(f"![Cluster {i} Diagram](cluster_{i}_diagram.png)\n\n")
                            f.write(f"[SVG-Version (interaktiv)](cluster_{i}_diagram.svg)\n\n")
                ...
                log_message("Stellen Sie sicher, dass Graphviz installiert ist (apt-get install graphviz)", "info")
...
def generate_table_diagrams(relationships, tables_with_relations, output_dir):
    ...
    try:
        ...
        if not dot_path:
            log_message("Graphviz ist nicht installiert. Bitte installieren Sie es mit 'apt-get install graphviz'.", "error")
            return
        ...
        with open(index_file, "w", encoding="utf-8") as f:
            ...
            f.write("Diese Diagramme zeigen die direkten Beziehungen jeder Tabelle.\n\n")
        ...
        for table in sorted(tables_with_relations):
            ...
            with open(dot_file, "w", encoding="utf-8") as df:
                ...
                df.write(f"  \"{table}\" [label=\"{table}\", style=\"filled\", fillcolor=\"lightblue\"];\n")
                ...
                for rel in direct_relations:
                    if rel["from_table"] == table:
                        ...
                        df.write(f"  \"{rel['to_table']}\" [label=\"{rel['to_table']}\"];\n")
                        df.write(f"  \"{table}\" -> \"{rel['to_table']}\" [label=\"{rel['from_column']} → {rel['to_column']}\"];\n")
                    elif rel["to_table"] == table:
                        related_tables.add(rel["from_table"])
                        df.write(f"  \"{rel['from_table']}\" [label=\"{rel['from_table']}\"];\n")
                        df.write(f"  \"{rel['from_table']}\" -> \"{table}\" [label=\"{rel['from_column']} → {rel['to_column']}\"];\n")
            ...
            try:
                subprocess.run([dot_path, "-Tpng", str(dot_file), "-o", str(png_file)], check=True)
                subprocess.run([dot_path, "-Tsvg", str(dot_file), "-o", str(svg_file)], check=True)
            except subprocess.CalledProcessError as e:
                log_message(f"Fehler beim Ausführen von Graphviz für {table}: {e}", "warning")
                continue
            
            # Füge Eintrag zur Indexdatei hinzu
            with open(index_file, "a", encoding="utf-8") as f:
                rel_count = len(direct_relations)
                f.write(f"| [{table}]({table}.md) | {rel_count} | [PNG](table_diagrams/{table}_relations.png) / [SVG](table_diagrams/{table}_relations.svg) |\n")
        ...
        log_message(f"Fehler bei der Erstellung der Tabellendiagramme: {str(e)}", "warning")
...
def generate_compact_schema_description():
    ...
    try:
        ...
        group_keywords = {
            "Finanzen": ["KONTO", "BUCHUNG", "SALDO", "ZAHLUNG", "BANK", "RUECKL", "UMLAGE", "ABRECHNUNG", "SOLLGEST"],
            "Immobilien": ["OBJEKT", "WOHNUNG", "HAUS", "LIEGEN", "ZAEHLER", "HEIZ", "VERBRAUCH"],
            "Personen": ["PERSON", "BEWOHNER", "EIGENTUEMER", "LIEFERAN", "MITARBEITER", "VERWALTER", "ADR"],
            "Verwaltung": ["VERWALT", "TERMIN", "AUFGABE", "VORGANG", "VERSAMMLUNG", "BESCHLUSS"],
        ...
        for yaml_file in yaml_output_dir.glob("*.yaml"):
            ...
            try:
                ...
                log_message(f"Fehler beim Laden von Beziehungen aus {yaml_file}: {e}", "warning", save_status=False)
            ...
            try:
                with open(yaml_file, "r", encoding="utf-8") as yf:
                    ...
                    for col in columns:
                        ...
                        if col_name in ["ID", "NR", f"{table_name}NR", f"{table_name}ID"]:
                    ...
                    for group, keywords in group_keywords.items():
                        if any(keyword in table_name for keyword in keywords) or \
                ...
                log_message(f"Fehler beim Verarbeiten von {yaml_file}: {e}", "warning", save_status=False)
        ...
        return True, f"Schema-Datei erstellt mit {len(tables_info)} Tabellen in {len(table_groups)} Gruppen."
    except Exception as e:
        log_message(f"Fehler bei der Erstellung der kompakten Schemabeschreibung: {e}", "error")
        return False, f"Fehler: {e}"
...
def get_relevant_schema_context(user_query, max_tables=5):
    ...
    try:
        ...
        for table_name, table_info in schema["tables"].items():
            ...
            if "buchung" in query_lower or "zahlung" in query_lower or "transaktion" in query_lower:
                if "BUCHUNG" in table_name or "ZAHLUNG" in table_name or "KONTO" in table_name:
                    score += 8
            
            if "adresse" in query_lower or "straße" in query_lower or "ort" in query_lower:
                if "ADR" in table_name or "PERSON" in table_name or "OBJEKT" in table_name:
                    score += 8
        ...
        relevant_tables = sorted(table_scores.items(), key=lambda x: x[1], reverse=True)[:max_tables]
        ...
        for rel in schema.get("relationships", []):
            if rel["from_table"] in related_tables and rel["to_table"] in related_tables:
        ...
        log_message(f"Fehler beim Abrufen des relevanten Schemakontexts: {e}", "warning", save_status=False)
...
with st.sidebar:
    st.header("Optionen")
    force_regenerate = st.checkbox("Alle Dokumente neu generieren (auch bereits vorhandene)", value=False)
    translate_english = st.checkbox("Englische Beschreibungen übersetzen", value=False)
    all_table_names = get_all_tables()
    table_subset = st.multiselect("Wähle Tabellen zur Generierung", all_table_names, default=all_table_names)
    ...
    with advanced_expander:
        ...
        sample_size = st.slider("Maximale Beispieldaten-Größe", 3, 30, DEFAULT_MAX_SAMPLE_SIZE)
        DEFAULT_MAX_SAMPLE_SIZE = sample_size
    
    st.info("Zweiphasen-Prozess: Nach Phase 1 (Initialdokumentation) wird in Phase 2 das gesammelte Wissen für Optimierung genutzt.")
        ...
        with open(os.path.join(logs_dir, "skipped_items.json"), "r") as f:
            try:
                ...
                if skipped.get("tables") or skipped.get("procedures"):
                    st.warning(f"⏩ Übersprungen wegen Größe: {len(skipped.get('tables', []))} Tabellen, {len(skipped.get('procedures', []))} Prozeduren")
                    with st.expander("Details zu übersprungenen Items"):
                        st.write("Tabellen:", ", ".join(skipped.get("tables", [])))
                        st.write("Prozeduren:", ", ".join(skipped.get("procedures", [])))
            except:
                pass
    ...
    with st.sidebar.expander("Generierungsstatus", expanded=True):
        ...
        status_data = {
            "Komponente": ["Tabellen", "Prozeduren", "Schema", "Beziehungsdiagramme", "DDL-Exports"],
            "Generiert": [
            ...
            "Phase 1": [status.tables_in_phase1, status.procedures_in_phase1, "-", "-", "-"],
            "Phase 2": [status.tables_in_phase2, status.procedures_in_phase2, "-", "-", "-"],
            "Übersprungen": [status.tables_skipped, status.procedures_skipped, "-", "-", "-"]
        }
        st.dataframe(pd.DataFrame(status_data).astype(str))
        
        if status.generated_details:
            details_data = [{"Name": key, "Phase": value} for key, value in status.generated_details.items()]
...
with phase1_tab:
    ...
    with col1:
        if st.button("🟢 ALLE Komponenten generieren (Phase 1)", key="generate_all_p1"):
            log_message("Durchlauf 1: Initiale YAML-Generierung für alle Komponenten", "info")
            
            if gen_tables:
                generate_yaml_batch(existing_yamls={}, force_regenerate=force_regenerate, phase="phase1", table_subset=table_subset, translate_english=translate_english)
            ...
            if gen_relations:
                try:
                    ...
                    log_message(f"Fehler beim Generieren der Beziehungsdiagramme: {e}", "warning")
            ...
            log_message("Phase 1 abgeschlossen. Du kannst jetzt optional Phase 2 starten.", "info")
            
            # Erstelle auch die kompakte Schemabeschreibung
            try:
                generate_compact_schema_description()
                log_message("Kompakte Schemabeschreibung für LLM-Chat erstellt.", "info")
            except Exception as e:
                log_message(f"Fehler bei der Erstellung der kompakten Schemabeschreibung: {e}", "warning")
    ...
    with col2:
        ...
        if st.button("📄 Nur Index.md generieren (Phase 1)", key="gen_index_p1"):
            ...
            if success:
                log_message(f"Phase 1: Index.md erfolgreich generiert. {message}", "info")
                st.success(f"✅ Index.md erfolgreich generiert: {schema_output_dir / 'index.md'}")
            else:
                log_message(f"Phase 1: Fehler bei der Generierung von Index.md: {message}", "error")
            
        if st.button("📊 Nur Beziehungsdiagramme generieren (Phase 1)", key="gen_rel_p1"):
            try:
                generate_relation_report()
...
with phase2_tab:
    ...
    existing_yamls_count = len(list(yaml_output_dir.glob("*.yaml"))) if yaml_output_dir.exists() else 0
    if existing_yamls_count == 0:
        st.warning("⚠️ Keine YAMLs aus Phase 1 gefunden! Bitte zuerst Phase 1 ausführen.")
    else:
        st.success(f"✅ {existing_yamls_count} YAML-Dateien aus Phase 1 gefunden. Phase 2 kann gestartet werden.")
    
    col1, col2 = st.columns(2)
    
    with col1:
        if st.button("🔄 ALLE Komponenten optimieren (Phase 2)", key="optimize_all_p2"):
            log_message("Phase 2: Optimierung aller Komponenten mit bestehendem Wissen", "info")
            
            new_yaml_context = load_existing_yaml_dict()
            
            if gen_tables:
                generate_yaml_batch(existing_yamls=new_yaml_context, force_regenerate=force_regenerate, phase="phase2", table_subset=table_subset, translate_english=translate_english)
            ...
            try:
                generate_compact_schema_description()
                log_message("Kompakte Schemabeschreibung für LLM-Chat aktualisiert.", "info")
            except Exception as e:
                log_message(f"Fehler bei der Aktualisierung der kompakten Schemabeschreibung: {e}", "warning")
    ...
    with col2:
        ...
        if st.button("📄 Nur Index.md generieren (Phase 2)", key="gen_index_p2"):
            success, message = create_index_file()
            if success:
                log_message(f"Phase 2: Index.md erfolgreich generiert. {message}", "info")
                st.success(f"✅ Index.md erfolgreich generiert: {schema_output_dir / 'index.md'}")
            else:
                log_message(f"Phase 2: Fehler bei der Generierung von Index.md: {message}", "error")
            
        if st.button("📊 Nur Beziehungsdiagramme optimieren (Phase 2)", key="opt_rel_p2"):
            try:
                generate_relation_report()
                log_message("Phase 2: Beziehungsdiagramme optimiert.", "info")
            except Exception as e:
                log_message(f"Fehler beim Optimieren der Beziehungsdiagramme: {e}", "warning")
...
with extras_tab:
    ...
    if st.button("🔍 Kompakte Schemabeschreibung generieren", key="gen_compact_schema"):
        with st.spinner("Generiere kompakte Schemabeschreibung..."):
            ...
            if success:
                st.success(message)
                st.info(f"Die Schemabeschreibung wurde in {schema_output_dir / 'compact_schema.json'} gespeichert.")
            else:
                st.error(message)
    ...
    with col2:
        if st.button("🧪 Nur YAML-Statistik generieren", key="gen_yaml_stats"):
            try:
                ...
                yamls = list(yaml_output_dir.glob("*.yaml")) if yaml_output_dir.exists() else []
                
                st.write(f"Dokumentierte Tabellen: {len([y for y in yamls if '_procedure' not in y.stem])}/{all_tables_count}")
                st.write(f"Dokumentierte Prozeduren: {len([y for y in yamls if '_procedure' in y.stem])}")
                ...
                st.write(f"Tabellen nach Priorität: Hoch={priority_counts['high']}, Mittel={priority_counts['medium']}, Niedrig={priority_counts['low']}")
                
                # Zeige ein Balkendiagramm
                df = pd.DataFrame.from_dict(priority_counts, orient='index', columns=['Anzahl'])
                st.bar_chart(df)
    ...
    if yamls:
        with st.expander("Statistik der vorhandenen Dokumentation", expanded=False):
            try:
                ...
                with col1:
                    st.metric("Dokumentierte Tabellen", 
                ...
                for yaml_file in yamls:
                    try:
                        ...
                        pass
                with col2:
                    st.metric("Leere Tabellen", empty_tables, 
...
Behaupte NIEMALS, dass du keinen Zugriff auf Informationen hast, die im Kontext enthalten sind.""",
    ...
    with st.form(key="chat_form"):
        ...
        with col2:
            include_yaml_info = st.checkbox("YAML-Dokumentation einbeziehen", value=True)
...
"""
                        ...
                        try:
                            ...
                            st.warning(f"Konnte keine Tabelleninformationen abrufen: {e}")
            ...
            if include_yaml_info and yaml_output_dir.exists():
                ...
                top_yamls = sorted(yaml_scores.items(), key=lambda x: x[1], reverse=True)[:3]
                    ...
                    try:
                        with open(yaml_file, "r", encoding="utf-8") as f:
                            ...
                            try:
                                ...
                                summary += f"Beschreibung: {data.get('description', '')}\n"
                                
                                # Füge wichtige Spalten hinzu
                                if "columns" in data and isinstance(data["columns"], list):
                                    summary += "Wichtige Spalten:\n"
                                    for col in data["columns"][:5]:  # Nur die ersten 5 Spalten
                                        if isinstance(col, dict):
                                ...
                                relevant_yamls.append(f"Informationen zu {yaml_file.stem}:\n{yaml_content[:300]}...")
                    except:
                        continue
