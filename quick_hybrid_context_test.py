...
class QuickHybridTester:
            ...
            try:
                ...
                if response and isinstance(response, dict):
                    ...
                    if result["sql_query"]:
                        ...
                        result["uses_aggregation"] = any(agg in sql_upper for agg in ["COUNT", "SUM", "AVG", "MAX", "MIN"])
                        result["table_count"] = len([table for table in ["BEWOHNER", "OBJEKTE", "KONTEN", "BEWADR", "EIGADR"] 
                                                    if table in sql_upper])
                    
                    self.logger.info(f"✅ Query successful in {result['response_time']:.1f}s")
    ...
    def run_concurrent_test(self, workers=3):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            future_to_task = {
                executor.submit(self.run_single_query, query_data, mode): (query_data, mode)
    ...
    def run_sequential_test(self):
        ...
        for i, query_data in enumerate(TEST_QUERIES, 1):
            ...
            for j, mode in enumerate(RETRIEVAL_MODES, 1):
                ...
                status = "✅" if result["success"] else "❌"
                self.logger.info(f"{status} Completed in {result['response_time']:.1f}s")
    ...
    def analyze_results(self):
        ...
        analysis["summary"]["overall_success_rate"] = len(successful_results) / len(self.results) * 100
        if successful_results:
            analysis["summary"]["average_response_time"] = sum(r["response_time"] for r in successful_results) / len(successful_results)
        ...
        for mode in RETRIEVAL_MODES:
            ...
            analysis["by_mode"][mode] = {
                ...
                "success_rate": len(mode_successful) / len(mode_results) * 100 if mode_results else 0,
                "avg_response_time": sum(r["response_time"] for r in mode_successful) / len(mode_successful) if mode_successful else 0,
                "context_usage": len([r for r in mode_successful if r.get("context_used", False)])
            }
        
        # Analysis by complexity
        for complexity in ["basic", "medium", "complex"]:
            complexity_results = [r for r in self.results if r.get("complexity") == complexity]
            ...
            if complexity_results:
                analysis["by_complexity"][complexity] = {
                    ...
                    "success_rate": len(complexity_successful) / len(complexity_results) * 100,
                    "avg_response_time": sum(r["response_time"] for r in complexity_successful) / len(complexity_successful) if complexity_successful else 0
                }
        ...
        for mode, metrics in analysis["by_mode"].items():
            ...
            score = metrics["success_rate"] - (metrics["avg_response_time"] / 10)  # Penalize slow responses
            if score > best_score:
                best_score = score
                best_mode = mode
    ...
    def save_results(self, results, analysis, filename_prefix="quick_hybrid_test"):
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- Success rate: {analysis['summary']['overall_success_rate']:.1f}%\n")
            f.write(f"- Average response time: {analysis['summary']['average_response_time']:.1f}s\n\n")
            
            f.write("## Results by Retrieval Mode\n")
            for mode, metrics in analysis["by_mode"].items():
                f.write(f"### {mode.upper()}\n")
                f.write(f"- Success rate: {metrics['success_rate']:.1f}%\n")
                f.write(f"- Average response time: {metrics['avg_response_time']:.1f}s\n")
                f.write(f"- Context usage: {metrics['context_usage']} queries\n\n")
            
            f.write("## Best Performing Mode\n")
            f.write(f"**{analysis['best_performing']['mode'].upper()}** (score: {analysis['best_performing']['score']:.1f})\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Quick Hybrid Context Test for WINCASA")
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(f"📊 Success rate: {analysis['summary']['overall_success_rate']:.1f}%")
        logger.info(f"⏱️  Average time: {analysis['summary']['average_response_time']:.1f}s")
        ...
        for mode in RETRIEVAL_MODES:
            ...
            logger.info(f"  {mode}: {metrics['success_rate']:.1f}% success, {metrics['avg_response_time']:.1f}s avg")
