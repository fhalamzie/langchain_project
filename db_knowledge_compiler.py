...
class DatabaseKnowledgeCompiler:
    ...
    def _load_schema_documentation(self):
        ...
        for file in os.listdir(self.schema_dir):
            if file.endswith('.md') and file != 'index.md' and not file.startswith('relationship'):
                ...
                if '_proc.md' in file:
                    ...
                    self.procedures[proc_name] = self._parse_procedure_doc(content, proc_name)
                elif file != 'index.md':  # Skip index.md as it's already processed
                    table_name = file.replace('.md', '')
                    if table_name not in self.tables:  # Only add if not already from index
                        self.tables[table_name] = self._parse_table_doc(content, table_name)
    ...
    def _parse_table_doc(self, content: str, table_name: str) -> Dict[str, Any]:
        ...
        context_match = re.search(r'### Geschäftskontext\s*\n(.*?)(?=\n###|\n##|\Z)', content, re.DOTALL)
        if context_match:
            table_info['business_context'] = context_match.group(1).strip()
        
        # Extract columns
        columns_match = re.search(r'## Spalten\s*\n(.*?)(?=\n##|\Z)', content, re.DOTALL)
        if columns_match:
                ...
                if '|' in line and not line.startswith('|---'):
                    ...
                    if len(parts) >= 3 and parts[0] != 'Spalte':
                        table_info['columns'].append({
                            'name': parts[0],
                            'type': parts[1],
                            'nullable': parts[2] == 'YES' if len(parts) > 2 else True
                        })
    ...
    def _parse_index_file(self, index_path: str):
        ...
        logger.info(f"Extracted from index.md: {len(table_matches)} tables, {len(proc_matches)} procedures")
    ...
    def _parse_procedure_doc(self, content: str, proc_name: str) -> Dict[str, Any]:
        ...
        # Extract parameters
        params_match = re.search(r'### Parameter\s*\n(.*?)(?=\n###|\n##|\Z)', content, re.DOTALL)
        if params_match:
            ...
            if file.endswith('.yaml'):
                ...
                if table_name in self.tables:
                    ...
                    if 'internal_conventions' in data:
                        self.tables[table_name]['conventions'] = data['internal_conventions']
    ...
    def _load_relationships(self):
        ...
        if os.path.exists(compact_schema_file):
            ...
            logger.info(f"Loaded {len(self.relationships)} relationships from compact_schema.json")
        else:
    ...
    def _analyze_table_importance(self):
        ...
        for table in self.tables:
            ...
            # Core entity bonus
            core_entities = ['KONTEN', 'OBJEKTE', 'EIGENTUEMER', 'BEWOHNER', 'VERTRAEGE']
        ...
        # Store top 20 tables
        self.top_tables = sorted(importance_scores.items(), key=lambda x: x[1], reverse=True)[:20]
    ...
    def _create_knowledge_base(self) -> Dict[str, Any]:
        ...
        knowledge_base = {
            ...
            'table_priorities': {
                ...
                'medium': [t for t, p in self.table_priorities.items() if p == 'medium'],
                'low': [t for t, p in self.table_priorities.items() if p == 'low']
            },
            'top_20_tables': [{'table': t[0], 'importance_score': t[1]} for t in self.top_tables],
            'core_entities': {
    ...
    def _identify_table_clusters(self) -> Dict[str, List[str]]:
        ...
        for table in self.tables:
            ...
            if any(kw in table_upper for kw in ['KONTO', 'SALDO', 'ZAHLUNG', 'SOLLSTELLUNG', 'ABRECHNUNG']):
                clusters['financial'].append(table)
            elif any(kw in table_upper for kw in ['OBJEKT', 'EINHEIT', 'GEBAEUDE', 'WOHNUNG']):
                clusters['property'].append(table)
            elif any(kw in table_upper for kw in ['EIGENTUEMER', 'BEWOHNER', 'PERSON', 'KUNDE']):
                clusters['person'].append(table)
            elif any(kw in table_upper for kw in ['VERTRAG', 'MIET']):
                clusters['contract'].append(table)
            elif any(kw in table_upper for kw in ['ZAEHLER', 'WASSER', 'STROM', 'HEIZUNG']):
...
"""
    ...
    def _get_key_procedures(self) -> List[Dict[str, str]]:
        ...
        financial_procs = ['SALDENLISTE', 'KONTOSALDO', 'OFFENE_SOLLSTELLUNGEN', 
                          'JOURNAL', 'OFFENE_POSTEN_LISTE']
        
        for proc_name in financial_procs:
            if proc_name in self.procedures:
                key_procs.append({
                    'name': proc_name,
                    'description': self.procedures[proc_name].get('description', '')[:200]
                })
    ...
    def _save_compiled_knowledge(self, knowledge_base: Dict[str, Any]):
        ...
        with open(prompt_path, 'w', encoding='utf-8') as f:
            ...
            for i, table_info in enumerate(knowledge_base['top_20_tables'][:10], 1):
                ...
                if table_name in self.tables:
                    f.write(f"{i}. {table_name}: {self.tables[table_name].get('description', 'No description')[:100]}...\n")
