...
def test_minimal_langchain():
    ...
    try:
        ...
        engine = create_engine("firebird+fdb://sysdba:masterkey@localhost:3050//home/projects/langchain_project/WINCASA2022.FDB")
        
        with engine.connect() as conn:
            result = conn.execute(text("SELECT COUNT(*) FROM WOHNUNG"))
            count = result.scalar()
            print(f"✅ LangChain Mode: Database connected via SQLAlchemy, {count} apartments found")
            return True
