...
class ProductionMonitor:
    ...
    def _save_metrics(self):
        ...
        recent_system = self.system_metrics[-1000:] if len(self.system_metrics) > 1000 else self.system_metrics
        with open(self.metrics_file, 'w') as f:
            json.dump([asdict(m) for m in recent_system], f, indent=2)
        
        # Save query metrics (keep last 500 entries)
        recent_queries = self.query_metrics[-500:] if len(self.query_metrics) > 500 else self.query_metrics
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                if proc.info['name'] == 'streamlit' or (
                    proc.info['cmdline'] and 'streamlit' in ' '.join(proc.info['cmdline'])
                ):
                    return "running"
            return "stopped"
    ...
    def log_query_metrics(self, query_text: str, processing_time: float, 
        ...
        if processing_time > 10.0:  # 10 second threshold
            self.logger.warning(f"Slow query detected: {processing_time:.2f}s - {query_text[:50]}...")
    ...
    def get_performance_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        avg_api_time = sum(m.api_response_time for m in recent_system if m.api_response_time) / max(1, len([m for m in recent_system if m.api_response_time]))
        
        # Query performance summary
        total_queries = len(recent_queries)
        successful_queries = len([q for q in recent_queries if q.success])
        avg_query_time = sum(q.processing_time for q in recent_queries) / max(1, total_queries)
        ...
        if latest_metrics:
            if (latest_metrics.cpu_percent > self.alert_thresholds['cpu_percent'] or
                ...
                not latest_metrics.database_accessible or
                latest_metrics.streamlit_status in ['stopped', 'unhealthy']):
        ...
        return {
            ...
            "system_metrics": {
                ...
                "database_accessible": latest_metrics.database_accessible if latest_metrics else False,
                "streamlit_status": latest_metrics.streamlit_status if latest_metrics else "unknown"
            },
            "query_metrics": {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--summary", action="store_true", help="Show performance summary and exit")
    ...
    if args.summary:
        ...
        if "error" in summary:
            ...
            print(f"Health Status: {status_icon.get(summary['health_status'], '❓')} {summary['health_status'].upper()}")
            ...
            print(f"   Database: {'✅ Accessible' if sys_metrics['database_accessible'] else '❌ Not Accessible'}")
            ...
            print(f"   Avg Processing Time: {query_metrics['average_processing_time']}s")
    else:
