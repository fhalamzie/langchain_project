...
def quick_optimization_demo():
    ...
    logger.info(f"Testing {len(QUICK_QUERIES)} queries x {len(RETRIEVAL_MODES)} modes = {len(QUICK_QUERIES) * len(RETRIEVAL_MODES)} tests")
    ...
    for i, query in enumerate(QUICK_QUERIES, 1):
        ...
        for mode in RETRIEVAL_MODES:
            ...
            try:
                ...
                found_petra = "Petra Nabakowski" in str(response) if "Marienstr" in query else False
                ...
                logger.info(f"✅ {mode}: {query_time:.1f}s" + (", Found Petra ✓" if found_petra else ""))
