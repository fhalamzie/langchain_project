...
class WincasaTestingFramework:
    ...
    def _execute_single_test(self, query: str, mode: str, timeout: int = 45) -> TestResult:
    ...
    def run_extensive_test(self, modes: List[str] = None, concurrent: bool = True) -> Dict[str, TestSummary]:
        """
    ...
    def run_baseline_test(self, modes: List[str] = None, concurrent: bool = True) -> Dict[str, Any]:
        """
    ...
    def _run_concurrent_tests(self, queries: List[str], mode: str, timeout: int) -> List[TestResult]:
    ...
    def _create_summary(self, results: List[TestResult], mode: str, test_level: str) -> TestSummary:
    ...
    def _generate_comparison_report(self, results: List[TestResult], modes: List[str]) -> Dict[str, Any]:
        ...
        for query, mode_results in query_comparison.items():
            ...
            for mode in modes:
                if mode in mode_results:
                    ...
                    row[f"{mode}_response"] = result.response[:100] + "..." if len(result.response) > 100 else result.response
        ...
        return {
            ...
            "overall_stats": {
                ...
                "test_duration": max(results, key=lambda x: x.execution_time).execution_time if results else 0
            }
        }
    
    def save_results(self, filename: str = None, test_level: str = "comprehensive") -> str:
    ...
    def generate_html_report(self, baseline_results: Dict[str, Any], filename: str = None) -> str:
        ...
        comparison_df = pd.DataFrame(baseline_results["detailed_comparison"]["query_comparison_matrix"])
        ...
        for mode, stats in baseline_results["detailed_comparison"]["mode_performance_stats"].items():
            html_content += f"""
                <div class="mode-card">
                    <h3>{mode.upper()}</h3>
...
def main():
    ...
    parser.add_argument("--level", choices=["quick", "extensive", "baseline"], 
    ...
    parser.add_argument("--html", action="store_true", 
                       help="Generate HTML report (baseline only)")
    ...
    if args.level == "quick":
        ...
        for mode, summary in results.items():
            print(f"{mode.upper()}: {summary.success_rate:.1%} success rate, "
                  f"avg time: {summary.average_execution_time:.1f}s")
