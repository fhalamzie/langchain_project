...
class QAEnhancer:
    ...
    def get_relevant_tables(self, query: str, max_tables: int = MAX_TABLES_IN_CONTEXT) -> List[str]:
        """
        ...
        table_similarity = [(self.table_names[i], similarities[i]) for i in range(len(self.table_names))]
    ...
    def get_table_context(self, table_names: List[str]) -> str:
        ...
        for table_name in table_names:
            if table_name in self.schema_data.get('tables', {}):
                ...
                description = table_info.get('description', 'Keine Beschreibung verfügbar.')
                business_context = table_info.get('business_context', '')
                    ...
                    if 'foreign_keys' in relations and relations['foreign_keys']:
                        ...
                        for fk in relations['foreign_keys']:
                            if isinstance(fk, dict):
                                ...
                                table_context += f"    - {field} → {ref_table}({ref_field})\n"
    ...
    def generate_sql(self, user_query: str, schema_context: str) -> str:
        ...
        messages = [
            ...
            {"role": "user", "content": f"Generiere eine SQL-Abfrage für: {user_query}"}
        ]
    ...
    def generate_natural_answer(self, user_query: str, sql: str, 
                               results: List[Dict[str, Any]], schema_context: str) -> str:
        """
        ...
        messages = [
            {"role": "system", "content": system_message},
            {"role": "user", "content": f"""Benutzerfrage: {user_query}
    ...
    def log_interaction(self, user_query: str, sql: str, success: bool, 
                       result_summary: str, answer: str) -> None:
        """
    ...
    def save_feedback(self, user_query: str, answer: str, feedback: str, 
                     rating: int, user_id: Optional[str] = None) -> None:
        """
    ...
    def process_query(self, user_query: str) -> Dict[str, Any]:
        ...
        if not relevant_tables:
            return {
                ...
                "raw_results": [],
                "formatted_results": "",
                "answer": "Ich konnte keine relevanten Tabellen für Ihre Anfrage identifizieren. Bitte stellen Sie Ihre Frage anders oder spezifischer."
            }
        ...
        if success:
            ...
            answer = self.generate_natural_answer(user_query, sql, result, schema_context)
        else:
        ...
        result_summary = f"{len(result) if success else 0} Ergebnisse" if success else result
