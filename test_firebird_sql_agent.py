...
class DummyLLM:
    def invoke(self, prompt: str) -> str:
        if "Variant 1 Text:" in prompt: # Mocking response for _generate_textual_responses
            return """
...
class TestFirebirdDocumentedSQLAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_db_conn_str = "firebird+fdb://dummy_user:dummy_password@localhost:3050/dummy.fdb"

    @patch('glob.glob')
    @patch('builtins.open', new_callable=mock_open)
    @patch('os.path.basename')
    def test_load_and_parse_documentation_success(self, mock_basename, mock_open_file, mock_glob):
        # --- Setup Mocks ---
        ...
        mock_ddl_procB_content = "CREATE PROCEDURE PROC_B_DDL (param1 VARCHAR(10)) AS BEGIN ... END;"
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_initialize_components', lambda self: None): # Prevent full init
            agent = FirebirdDocumentedSQLAgent(db_connection_string=self.mock_db_conn_str, llm=self.mock_llm)
        ...
        # MD
        doc_index_md = next(d for d in parsed_docs if d.metadata["source"] == "index.md")
        self.assertIn("# Main Index", doc_index_md.page_content)
        self.assertEqual(doc_index_md.metadata["type"], "schema_markdown")

        # YAML (Table)
        doc_yaml_tableA = next(d for d in parsed_docs if d.metadata["source"] == "tableA.yaml")
        ...
        # YAML (Procedure)
        doc_yaml_procB = next(d for d in parsed_docs if d.metadata["source"] == "procB.yaml")
        self.assertIn("Entity Name: PROC_B_FROM_YAML", doc_yaml_procB.page_content)
        self.assertIn("param1", doc_yaml_procB.page_content) # Check if parameters are mentioned
        self.assertEqual(doc_yaml_procB.metadata["type"], "yaml_definition")

        # SQL (Table DDL)
        doc_ddl_tableA = next(d for d in parsed_docs if d.metadata["source"] == "tableA_ddl.sql")
        ...
        # SQL (Procedure DDL)
        doc_ddl_procB = next(d for d in parsed_docs if d.metadata["source"] == "procB_procedure.sql")
    ...
    def test_initialize_components_faiss_mode(self, MockOpenAIEmbeddings, MockFaissDocumentationRetriever):
        # Setup Mocks
        mock_faiss_retriever_instance = MockFaissDocumentationRetriever.return_value
        MockOpenAIEmbeddings.return_value = MagicMock() # Mock OpenAIEmbeddings instance
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_load_and_parse_documentation', lambda self: setattr(self, 'parsed_docs', [Document(page_content="doc1")])):
    ...
    def test_setup_sql_agent_success(self, MockSQLDatabase, MockSQLDatabaseToolkit, MockCreateSqlAgent):
        # Setup Mocks
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_load_and_parse_documentation', lambda self: setattr(self, 'parsed_docs', [Document(page_content="doc1")])):
            with patch.object(FirebirdDocumentedSQLAgent, '_initialize_components', lambda self: None): # Prevent retriever init
                agent = FirebirdDocumentedSQLAgent(
                    db_connection_string=self.mock_db_conn_str,
        ...
        MockSQLDatabase.assert_called_once_with(
            ...
            engine_args={'connect_args': {'timeout': 30}}, # Check for specific engine_args if any
        ...
        MockSQLDatabaseToolkit.assert_called_once_with(db=mock_db_instance, llm=self.mock_llm)
        ...
        self.assertIn("You are an agent designed to interact with a Firebird SQL database", kwargs.get("prefix", ""))
        self.assertIn("WINCASA2022", kwargs.get("prefix", "")) # Check for specific context words
        self.assertTrue(kwargs.get("verbose", False)) # or check its actual value if set
        # Check agent_executor_kwargs if specific ones are set
        self.assertEqual(kwargs.get("agent_executor_kwargs", {}).get("handle_parsing_errors"), True)
    ...
    def test_setup_sql_agent_no_db_conn(self, mock_log_warning, MockSQLDatabase, MockSQLDatabaseToolkit, MockCreateSqlAgent):
        ...
        mock_log_warning.assert_called_with("SQL Agent not created due to missing DB connection string or LLM.")

    @patch('firebird_sql_agent.create_sql_agent')
    @patch('firebird_sql_agent.SQLDatabaseToolkit')
    @patch('firebird_sql_agent.SQLDatabase')
    @patch('logging.warning') # To check for the warning log
    def test_setup_sql_agent_no_llm(self, mock_log_warning, MockSQLDatabase, MockSQLDatabaseToolkit, MockCreateSqlAgent):
    ...
    @patch('firebird_sql_agent.OpenAIEmbeddings')
    @patch('firebird_sql_agent.FaissDocumentationRetriever')
    def test_query_faiss_mode(self, MockFaissRetriever, MockOpenAIEmbeddings, MockSQLDatabase, MockSQLToolkit, MockCreateAgent):
        # --- Setup Mocks ---
        ...
        mock_sql_agent_instance.invoke.return_value = {
            ...
            "intermediate_steps": [("some_action", "SELECT * FROM DUMMY_TABLE;")] # Example intermediate step
        }
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_load_and_parse_documentation', lambda self: setattr(self, 'parsed_docs', [Document(page_content="parsed_doc_content")])):
            agent = FirebirdDocumentedSQLAgent(
                db_connection_string=self.mock_db_conn_str,
        ...
        with patch.object(agent, '_generate_textual_responses', return_value=expected_responses) as mock_generate_responses:
        ...
        mock_faiss_retriever_instance.get_relevant_documents.assert_called_once_with(nl_query)
        ...
        call_args = mock_sql_agent_instance.invoke.call_args[0][0] # Get the first positional argument (the dict)
    ...
    def test_query_no_sql_agent(self, mock_log_error):
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_load_and_parse_documentation', lambda self: setattr(self, 'parsed_docs', [Document(page_content="doc1")])):
            ...
            with patch.object(FirebirdDocumentedSQLAgent, '_initialize_components', lambda self: setattr(self, 'active_retriever', MagicMock())):
                 agent = FirebirdDocumentedSQLAgent(
        ...
        with patch.object(agent, '_generate_textual_responses') as mock_generate_responses:
            nl_query = "Some query"
        ...
        mock_log_error.assert_called_with("SQL Agent is not initialized. Cannot execute query.")
    ...
    def test_generate_textual_responses_success(self):
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_load_and_parse_documentation', lambda self: None):
            with patch.object(FirebirdDocumentedSQLAgent, '_initialize_components', lambda self: None):
                with patch.object(FirebirdDocumentedSQLAgent, '_setup_sql_agent', lambda self: None):
                    agent = FirebirdDocumentedSQLAgent(db_connection_string=None, llm=self.mock_llm) # self.mock_llm is DummyLLM
        ...
        sql_agent_response = {
            ...
            "intermediate_steps": [
                ...
                ("some_action_step", "Final Answer: Some answer based on SQL") # if SQL is not directly in output
            ],
            "sql_query": "SELECT DUMMY_COL FROM DUMMY_TABLE;" # Assuming SQL is directly available or extracted
        }
        ...
        sql_agent_response_intermediate = {
            ...
            "intermediate_steps": [
                ("Action", {"query": "SELECT * FROM DUMMY_INTERMEDIATE;"}), # Langchain AgentExecutor format
            ]
        }
    ...
    def test_generate_textual_responses_no_llm(self, mock_log_error):
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_load_and_parse_documentation', lambda self: None):
            with patch.object(FirebirdDocumentedSQLAgent, '_initialize_components', lambda self: None):
                with patch.object(FirebirdDocumentedSQLAgent, '_setup_sql_agent', lambda self: None):
                    agent = FirebirdDocumentedSQLAgent(db_connection_string=None, llm=None) # LLM is None

        self.assertIsNone(agent.llm) # Verify precondition

        # Input data
        nl_query = "Query when LLM is None"
        sql_agent_response = {"output": "Some SQL output", "sql_query": "SELECT 1;"} # sql_query for SQL extraction
        ...
        self.assertEqual(r3, "-")
        mock_log_error.assert_called_with("LLM is not initialized. Cannot generate textual responses.")
    ...
    def test_generate_textual_responses_agent_failure_case(self, mock_log_error):
        ...
        with patch.object(FirebirdDocumentedSQLAgent, '_load_and_parse_documentation', lambda self: None):
            with patch.object(FirebirdDocumentedSQLAgent, '_initialize_components', lambda self: None):
                with patch.object(FirebirdDocumentedSQLAgent, '_setup_sql_agent', lambda self: None):
                    agent = FirebirdDocumentedSQLAgent(db_connection_string=None, llm=self.mock_llm)
        ...
        self.assertEqual(r3, "-")
        mock_log_error.assert_called_with("SQL agent response is None. Cannot generate textual responses.")
        ...
        self.assertEqual(sql, "No SQL query extracted from agent response.") # Default if no SQL found
        self.assertEqual(r1, "Error reported by SQL agent: Simulated agent error: Query timed out")
        ...
        mock_log_error.assert_any_call("Error found in SQL agent response: {'error': 'Simulated agent error: Query timed out'}")
