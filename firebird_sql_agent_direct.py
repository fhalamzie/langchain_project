...
class FirebirdDirectSQLAgent:
    ...
    def _initialize_components(self):
        ...
        if self.retrieval_mode == "faiss":
            ...
            self.active_retriever = None
            print("None mode initialized - no document retrieval, global context fallback only.")
...
Thought:{agent_scratchpad}"""
    ...
    def query(
        ...
        elif current_retrieval_mode == "langgraph" and hasattr(self, 'langgraph_workflow') and self.langgraph_workflow:
        ...
        # Special handling for LangGraph workflow mode
            ...
            try:
                ...
                workflow_result = self.langgraph_workflow.process_query(natural_language_query)
                workflow_execution_time = time.time() - workflow_start_time
                
                print(f"✅ LangGraph workflow completed in {workflow_execution_time:.2f}s")
                
                # Return workflow result directly
                return {
                    "answer": workflow_result.get("answer", "No answer from workflow"),
                    "sql_query": workflow_result.get("sql_query", "Generated by LangGraph workflow"),
                    "raw_results": workflow_result.get("raw_results", []),
                    "retrieval_mode": current_retrieval_mode,
                    "documents_retrieved": workflow_result.get("documents_retrieved", 0),
