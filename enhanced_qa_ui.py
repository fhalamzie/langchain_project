...
from db_executor import (  # execute_sql für Rohdaten
    execute_sql,
    get_all_tables,
    results_to_dataframe,
)
from firebird_sql_agent_direct import FirebirdDirectSQLAgent
from phoenix_monitoring import PhoenixMonitor, get_monitor

# Importiere lokale Module
from qa_enhancer import (  # Behalten wir vorerst für Feedback-Speicherung, falls nötig
    QAEnhancer,
)
...
@st.cache_resource
def get_firebird_sql_agent(
    retrieval_mode: str = "faiss", use_enhanced_knowledge: bool = True
):
    ...
    st.info(
        f"Retrieval-Modus: {retrieval_mode}, Enhanced Knowledge: {'Aktiviert' if use_enhanced_knowledge else 'Deaktiviert'}"
    )
    try:
...
def create_enhanced_qa_tab():
    ...
    **🎉 Neue Features:**
    ...
    if agent is None:
        st.error(
            "❌ Direkte FDB SQL Agent konnte nicht geladen werden. Bitte überprüfen Sie die Konfiguration und Fehlermeldungen."
        )
    ...
    with chat_container:
        for i, entry in enumerate(st.session_state.enhanced_chat_history):
            ...
            if entry.get("error"):
                ...
                with st.expander("🔍 Details zur Agenten-Verarbeitung", expanded=False):
                    ...
                    if "retrieved_context" in entry and entry["retrieved_context"]:
                        ...
                        st.text(
                            entry["retrieved_context"][:1000] + "..."
                            if len(entry["retrieved_context"]) > 1000
                            else entry["retrieved_context"]
                        )
                    ...
                    if "detailed_steps" in entry and entry["detailed_steps"]:
                        st.markdown(
                            "**🔧 Detaillierte Agent-Schritte (Direkte FDB-Schnittstelle):**"
                        )
                        for i, step in enumerate(entry["detailed_steps"]):
                            with st.expander(
                                f"Schritt {i+1}: {step.get('action', {}).get('tool', 'Unbekannt')}",
                                expanded=False,
                            ):
                                if "action" in step:
                                    st.json(step["action"])
                ...
                if "text_variants" in entry and entry["text_variants"]:
                    ...
                    variant_tabs = st.tabs(
                        [variant["variant_name"] for variant in entry["text_variants"]]
                    )
                    for tab, variant_data in zip(variant_tabs, entry["text_variants"]):
                        with tab:
                            st.markdown(variant_data["text"])
                elif (
                    "agent_final_answer" in entry
                ):  # Fallback, falls text_variants nicht da sind
                ...
                if monitor:
                    with st.expander("📊 Query Monitoring", expanded=False):
                        ...
                        with col3:
                            st.metric(
                                "Avg Cost",
                                f"${metrics['total_cost_usd']/max(1, metrics['total_queries']):.4f}",
                            )

                        if (
                            "retrieval_performance" in metrics
                            and metrics["retrieval_performance"]
                        ):
                            st.markdown("**Retrieval Performance:**")
                            for mode, stats in metrics["retrieval_performance"].items():
                                if stats["total_retrievals"] > 0:
                                    st.write(
                                        f"- **{mode.upper()}**: {stats['successful_retrievals']}/{stats['total_retrievals']} successful, "
            ...
            if entry.get("text_variants") and entry["text_variants"]:
                feedback_target_answer = entry["text_variants"][0][
                    "text"
    ...
    with st.form(key="enhanced_qa_form"):
        ...
        if submit_button and user_query:
            if agent is None:
                st.error(
                    "❌ Direkte FDB SQL Agent ist nicht verfügbar. Anfrage kann nicht bearbeitet werden."
                )
                return

            with st.spinner(
                "🔄 Direkte FDB Agent verarbeitet Ihre Anfrage... Bitte haben Sie einen Moment Geduld."
            ):
                ...
                history_entry = {
                    "natural_language_query": (
                        user_query
                    ),  # Stelle sicher, dass dieser Schlüssel immer da ist
