            ...
            try:
                ...
                if "firebird+fdb://" in connection_string:
                    ...
                    if len(parts) >= 2:
                        ...
                        server_connection = (
                            f"firebird+fdb://{user}:{password}@localhost:3050/{db_path}"
                        )
...
def test_langchain_integration():
    ...
    try:
        ...
        docs = retriever.retrieve_documents(
            "How many tables are in the database?", max_docs=3
        )
...
def main():
    ...
    print(
        f"  Firebird server: {'✅ Running' if server_running else '❌ Not available'}"
    )
    print(
        f"  LangChain integration: {'✅ Working' if integration_success else '❌ Failed'}"
    )
