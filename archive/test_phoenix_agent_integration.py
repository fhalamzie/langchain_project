...
class TestFirebirdAgentPhoenixIntegration(unittest.TestCase):
    ...
    @patch(
        "firebird_sql_agent_direct.FirebirdDirectSQLAgent._load_and_parse_documentation"
    )
    ...
    def test_query_tracking(self, mock_init_components, mock_load_docs):
        ...
        agent.sql_agent.invoke.return_value = {"output": "Test result"}
    ...
    def test_callback_handler_llm_tracking(self):
        ...
        # Simulate LLM start
        handler.on_llm_start(serialized={"name": "gpt-4"}, prompts=["Test prompt"])
...
class TestEnhancedRetrieversPhoenixIntegration(unittest.TestCase):
    ...
    def test_enhanced_multi_stage_retriever_tracking(self, mock_faiss, mock_embeddings):
        ...
        retriever = EnhancedMultiStageRetriever(
            parsed_docs=self.test_docs, openai_api_key="test-key"
        )
    ...
    def test_enhanced_faiss_retriever_tracking(
        self, mock_get_docs, mock_init, mock_embeddings
    ):
    ...
    def test_retrieval_error_tracking(self):
        ...
        with patch(
            "enhanced_retrievers.EnhancedMultiStageRetriever._stage1_schema_retrieval"
        ) as mock_stage1:
            mock_stage1.side_effect = Exception("Test error")

            retriever = EnhancedMultiStageRetriever(
                parsed_docs=[], openai_api_key="test-key"
            )
