...
def test_knowledge_compiler():
    ...
    if kb_path.exists():
        ...
        for i, table_info in enumerate(kb["top_20_tables"][:5], 1):
            print(
                f"  {i}. {table_info['table']} (score: {table_info['importance_score']})"
            )
    else:
...
def test_enhanced_retriever():
    ...
    try:
        ...
        enhanced_retriever = EnhancedMultiStageRetriever(
            parsed_docs=parsed_docs, openai_api_key=os.getenv("OPENAI_API_KEY", "")
        )
        ...
        for query in test_queries:
            ...
            if docs:
                first_doc = docs[0]
                print(
                    f"   First doc source: {first_doc.metadata.get('source', 'unknown')}"
                )
            ...
            try:
                ...
                if result.get("error"):
                    ...
                    if result.get("agent_final_answer"):
                        print(
                            f"   Answer preview: {result['agent_final_answer'][:100]}..."
                        )

                    if result.get("text_variants"):
                        print(
                            f"   Generated {len(result['text_variants'])} text variants"
                        )
