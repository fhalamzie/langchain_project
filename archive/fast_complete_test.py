    ...
    for i, query in enumerate(TEST_QUERIES, 1):
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                elif "bäuminghausstr" in query.lower() and any(
                    name in answer_text for name in ["bewohner", "mieter", "name"]
                ):
                    result["found_expected"] = True
                    print(f"  ✓ Found residents in Bäuminghausstr")
                elif "schmiedestr" in query.lower() and any(
                    name in answer_text for name in ["bewohner", "mieter", "name"]
                ):
                    result["found_expected"] = True
                ...
                elif "miete" in query.lower() and (
                    "€" in answer_text
                    or "euro" in answer_text
                    or any(c.isdigit() for c in answer_text)
                ):
                    result["found_expected"] = True
...
def run_fast_complete_test():
    ...
    for mode in RETRIEVAL_MODES:
        ...
        total_time = sum(
            r["execution_time"] for r in mode_results if r["execution_time"]
        )
        ...
        mode_summaries[mode] = {
            ...
            "avg_time": avg_time,
            "success_rate": (
                len(successful) / len(mode_results) * 100 if mode_results else 0
            ),
            "accuracy_rate": (
                len(expected_found) / len(mode_results) * 100 if mode_results else 0
            ),
        }
    ...
    print(f"\nMODE COMPARISON:")
    print(
        f"{'Mode':<10} {'Success':<8} {'Accuracy':<10} {'Avg Time':<10} {'Total Time':<12}"
    )
    print("-" * 60)

    for mode, summary in mode_summaries.items():
        print(
            f"{mode:<10} {summary['success_rate']:>6.1f}%  {summary['accuracy_rate']:>8.1f}%  {summary['avg_time']:>8.1f}s  {summary['total_time']:>10.1f}s"
        )

    # Find best mode
    best_mode = max(
        mode_summaries.items(), key=lambda x: (x[1]["accuracy_rate"], -x[1]["avg_time"])
    )
