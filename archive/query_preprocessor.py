...
class QueryPreprocessor:
    ...
    def _initialize_nlp_patterns(self):
        ...
        self.address_patterns = [
            (
                r"\b([a-zA-ZäöüÄÖÜß]+str\.?|[a-zA-ZäöüÄÖÜß]+straße)\s*(\d+[a-z]?)\b",
                "street_number",
            ),
            (r"\b(\d{5})\s+([a-zA-ZäöüÄÖÜß]+)\b", "postal_city"),
            (
                r"\b([a-zA-ZäöüÄÖÜß]+weg|[a-zA-ZäöüÄÖÜß]+platz|[a-zA-ZäöüÄÖÜß]+gasse)\s*(\d+[a-z]?)\b",
                "address_type",
            ),
    ...
    def _extract_entities(self, query: str) -> List[Dict[str, str]]:
        ...
        for term, table in self.entity_mappings.items():
            if term in query_lower:
                entities.append(
                    {"term": term, "table": table, "type": "business_entity"}
                )
        ...
        for pattern, time_type in self.time_patterns:
            matches = re.finditer(pattern, query, re.IGNORECASE)
            for match in matches:
                entities.append({"term": match.group(0), "type": f"time_{time_type}"})
    ...
    def _identify_tables(self, query: str, entities: List[Dict[str, str]]) -> List[str]:
        ...
        if not tables:
            ...
            if any(
                term in query_lower
                for term in ["zahlung", "payment", "saldo", "balance", "konto"]
            ):
            ...
            # Property context
                ...
                term in query_lower
                for term in ["objekt", "property", "wohnung", "apartment"]
            ):
            ...
            # Person context
                ...
                term in query_lower
                for term in ["eigentümer", "owner", "mieter", "tenant"]
            ):
            ...
            for j in range(i + 1, len(tables)):
                try:
                    ...
                    path = nx.shortest_path(
                        self.relationship_graph, tables[i], tables[j]
                    )
                    ...
                    logger.warning(
                        f"No relationship path found between {tables[i]} and {tables[j]}"
                    )
    ...
    def _enrich_query(
        self, query: str, tables: List[str], join_paths: List[Dict], intent: Dict
    ) -> str:
        ...
        if join_paths:
            ...
            for path in join_paths[:2]:  # Limit paths
                for join in path["joins"]:
                    context_parts.append(f"- {join['relationship']}")
    ...
    def _generate_suggestions(self, tables: List[str], intent: Dict) -> List[str]:
        ...
        if "KONTEN" in tables:
            suggestions.append(
                "Consider using KONTOSALDO procedure for balance calculations"
            )

        if "EIGENTUEMER" in tables and "OBJEKTE" in tables:
            suggestions.append(
                "Join through VEREIG table for owner-property relationships"
            )
        ...
        if intent["grouping"] and "OBJEKTE" in tables:
            suggestions.append(
                "Consider grouping by OBJEKTE.VERWNR for management units"
            )
