...
class ProductionConfig:
    ...
    def setup_production_environment(cls) -> Dict[str, Any]:
        ...
        config["database"] = {
            ...
            "connection_string": (
                f"firebird+fdb://sysdba:masterkey@/{cls.DATABASE_PATH}"
            ),
        }
        ...
        config["streamlit"] = {
            ...
            "command": (
                f"streamlit run enhanced_qa_ui.py --server.port {cls.STREAMLIT_PORT} --server.address {cls.STREAMLIT_HOST}"
            ),
        }
    ...
    def _validate_environment(cls) -> Dict[str, bool]:
        ...
        # Check documentation
        validation["yaml_docs_exist"] = (
            cls.YAML_DOCS_DIR.exists()
            and len(list(cls.YAML_DOCS_DIR.glob("*.yaml"))) > 0
        )
        validation["schema_docs_exist"] = cls.SCHEMA_DOCS_DIR.exists()

        # Check Python environment
        validation["virtual_env_active"] = (
            hasattr(os, "real_prefix")
            or (hasattr(os, "base_prefix") and os.base_prefix != os.prefix)
        ...
        if cls.OPENAI_ENV_PATH.exists():
            try:
                with open(cls.OPENAI_ENV_PATH, "r") as f:
                    ...
                    api_keys["openai_configured"] = (
                        "OPENAI_API_KEY=" in content and len(content.split("=")[1]) > 10
                    )
            except Exception:
        ...
        if cls.OPENROUTER_ENV_PATH.exists():
            try:
                with open(cls.OPENROUTER_ENV_PATH, "r") as f:
                    content = f.read().strip()
                    api_keys["openrouter_configured"] = (
                        "OPENROUTER_API_KEY=" in content
                        and len(content.split("=")[1]) > 10
                    )
            except Exception:
    ...
    def _get_recommendations(
        cls, validation: Dict[str, bool], api_keys: Dict[str, bool]
    ) -> List[str]:
        ...
        if not validation.get("database_exists", False):
            recommendations.append(
                "❌ Database file WINCASA2022.FDB not found in project root"
            )

        if not validation.get("fb_client_exists", False):
            recommendations.append(
                "❌ Firebird client library not found at ./lib/libfbclient.so"
            )

        if not api_keys.get("openai_configured", False):
            recommendations.append(
                "❌ OpenAI API key not configured in /home/envs/openai.env"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Requirements Met: {status['critical_requirements_met']}/{status['total_requirements']}"
    )
    print()
