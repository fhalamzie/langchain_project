...
class TestPhoenixUIIntegration(unittest.TestCase):
    ...
    def test_phoenix_sidebar_display(self, mock_get_phoenix):
        ...
        # Mock session state
        self.mock_st.session_state = {"enhanced_chat_history": [], "feedback_given": {}}
        ...
        markdown_calls = [
            call for call in self.mock_st.sidebar.method_calls if call[0] == "markdown"
        ]
        self.assertTrue(any("AI Observability" in str(call) for call in markdown_calls))

        # Should show success status
        success_calls = [
            call for call in self.mock_st.sidebar.method_calls if call[0] == "success"
        ]
        self.assertTrue(
            any("Phoenix Monitoring aktiv" in str(call) for call in success_calls)
        )

        # Should display metrics
        metric_calls = [
            call for call in self.mock_st.sidebar.method_calls if call[0] == "metric"
        ]
    ...
    def test_phoenix_metrics_in_query_results(self, mock_get_phoenix):
        ...
        expander_calls = [
            call for call in self.mock_st.method_calls if call[0] == "expander"
        ]
    ...
    def test_phoenix_unavailable_handling(self, mock_get_phoenix):
        ...
        warning_calls = [
            call for call in self.mock_st.sidebar.method_calls if call[0] == "warning"
        ]
        self.assertTrue(
            any(
                "Phoenix Monitoring nicht verfügbar" in str(call)
...
class TestPhoenixMetricsDisplay(unittest.TestCase):
    ...
    def test_empty_metrics_handling(self):
        ...
        avg_cost = empty_metrics["total_cost_usd"] / max(
            1, empty_metrics["total_queries"]
        )
