...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f'retrieval_test_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        ),
...
class RetrievalTestRunner:
    def __init__(self, timeout_seconds: int = 180):  # Timeout erhöht auf 180 Sekunden
        ...
        logger.info(
            f"Phoenix monitoring initialized. Dashboard: {self.monitor.session.url if self.monitor and self.monitor.session else 'N/A'}"
        )
    ...
    def initialize_agents(self):
        ...
        for model_name in MODEL_NAMES:
            for mode in RETRIEVAL_MODES:
                ...
                initialized_count += 1
                logger.info(
                    f"Initializing agent {initialized_count}/{total_agents_to_init}: {model_name} (mode: {mode})"
                )
                try:
                    # Ensure previous connections are closed
                    if (
                        hasattr(self, "current_agent_for_init")
                        and self.current_agent_for_init
                    ):
                        if (
                            hasattr(self.current_agent_for_init, "db_interface")
                            and self.current_agent_for_init.db_interface
                        ):
    ...
    def execute_query_with_timeout(
        ...
        try:
            ...
            if isinstance(response, dict):
                result["answer"] = response.get(
                    "agent_final_answer", response.get("output", str(response))
                )
                result["sql_query"] = response.get(
                    "generated_sql", response.get("sql_query", "N/A")
                )
    ...
    def run_all_tests(self):
        ...
        for query in TEST_QUERIES:
            ...
            query_results_for_all_models = (
                []
            )  # Store results for this query across all models/modes
            ...
            for model_name in MODEL_NAMES:
                ...
                if result["success"]:
                    logger.info(
                        f"✓ Success - Time: {result['execution_time']:.2f}s, Rows: {result['row_count']}"
                    )
                    if (
                        result["sql_query"] and result["sql_query"] != "N/A"
                    ):  # Zusätzliche Prüfung auf None
    ...
    def _compare_query_results(self, query: str, results: List[Dict]):
        ...
        if len(successful_modes) > 1:
            ...
            if len(set(row_counts.values())) == 1:
                logger.info(
                    f"✓ All modes returned same row count: {list(row_counts.values())[0]}"
                )
            else:
    ...
    def generate_report(self):
        ...
        for model_name in MODEL_NAMES:
            ...
            for mode in RETRIEVAL_MODES:
                ...
                avg_time = sum(
                    r["execution_time"] for r in specific_results if r["success"]
                ) / max(successful, 1)

                summary["model_mode_statistics"][model_name][mode] = {
                    "success_rate": (
                        successful / len(specific_results) * 100
                        if specific_results
                        else 0
                    ),
    ...
    def _print_summary(self, summary: Dict):
        ...
        for model_name, model_stats in summary["model_mode_statistics"].items():
            ...
            for mode, stats in model_stats.items():
                ...
                print(
                    f"    Successful: {stats['successful_queries']}/{summary['total_queries']}"
                )
