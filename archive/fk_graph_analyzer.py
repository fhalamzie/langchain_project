...
class FKGraphAnalyzer:
    ...
    def initialize_from_global_context(self) -> None:
        ...
        try:
            ...
            print(
                f"✓ FKGraphAnalyzer: Initialized with {len(core_relationships)} relationships"
            )

        except ImportError:
            print(
                "⚠️ FKGraphAnalyzer: global_context.py not found, using fallback relationships"
            )
    ...
    def _initialize_fallback_relationships(self) -> None:
        ...
        self._is_initialized = True
        print(
            f"✓ FKGraphAnalyzer: Initialized with {len(fallback_relationships)} fallback relationships"
        )
    ...
    def find_join_path(self, from_table: str, to_table: str) -> Optional[JoinPath]:
        ...
        try:
            ...
            path = nx.shortest_path(
                self.graph, from_table.upper(), to_table.upper(), weight="weight"
            )
            ...
            for i in range(len(path) - 1):
                ...
                if self.graph.has_edge(source, target):
                    ...
                    join_sql = (
                        f"JOIN {target} ON {source}.{from_col} = {target}.{to_col}"
                    )
    ...
    def get_all_related_tables(self, tables: List[str], max_hops: int = 2) -> Set[str]:
        """
    ...
    def generate_join_sql(
        self, tables: List[str], base_table: Optional[str] = None
    ) -> str:
        """
    ...
    def analyze_query_complexity(self, tables: List[str]) -> Dict:
        ...
        if total_complexity > 5.0:
            analysis["optimization_suggestions"].append(
                "Consider using a different base table"
            )
        if len(analysis["missing_relationships"]) > 0:
            analysis["optimization_suggestions"].append(
                "Some table relationships are missing or indirect"
            )
        if len(tables) > 5:
            analysis["optimization_suggestions"].append(
            ...
            "edges": [
                {
                    ...
                    "relationship_type": (
                        self.graph[edge[0]][edge[1]].get("relationship_type", "unknown")
                    ),
                }
...
if __name__ == "__main__":
    ...
    complexity = analyzer.analyze_query_complexity(
        ["BEWOHNER", "OBJEKTE", "KONTEN", "BUCHUNG"]
    )
