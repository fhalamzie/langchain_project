...
def get_sql_agent(retrieval_mode: str = "enhanced"):
    ...
    try:
        ...
        if "SQLCODE: -902" in error_msg and "Database already opened" in error_msg:
            st.error(
                "🔒 **Database Lock Error**: Die Datenbank ist bereits von einem anderen Prozess geöffnet."
            )
            st.info(
                "💡 **Lösung**: Stoppen Sie laufende Tests oder warten Sie bis diese beendet sind."
            )
                    ...
                    if isinstance(step, dict):
                        ...
                        if "output" in step:
                            with st.expander(f"📤 Output (Step {i})", expanded=False):
                                st.text(
                                    str(step["output"])[:1000] + "..."
                                    if len(str(step["output"])) > 1000
        ...
        if "query_results" in response:
            ...
            if results:
                ...
                if (
                    ...
                    st.markdown(
                        f"**Spalten:** {len(results[0])} ({', '.join(list(results[0].keys())[:5])}{'...' if len(results[0]) > 5 else ''})"
                    )
    ...
    if "query_results" in response and response["query_results"]:
        ...
        try:
            if isinstance(results, list) and len(results) > 0:
                ...
                if len(df) > 100:
                    st.warning(
                        f"Große Ergebnismenge ({len(df)} Zeilen). Zeige erste 100 Zeilen."
                    )
...
def main():
    ...
    st.markdown(
        "**Natürlichsprachige Datenbankabfragen mit KI-gestützter SQL-Generierung**"
    )
    ...
    with col1:
        ...
        with col_btn1:
            submit_button = st.button(
                "🚀 Abfrage starten", type="primary", use_container_width=True
            )
    ...
    if submit_button and query.strip():
        ...
        if agent is None:
            st.error(
                "❌ Agent konnte nicht initialisiert werden. Bitte prüfen Sie die Konfiguration."
            )
            return
        ...
        with st.spinner(f"🔄 Verarbeite Anfrage mit {retrieval_mode.upper()} Modus..."):
            ...
            try:
                ...
                # Display results
                st.success(
                    f"✅ Abfrage erfolgreich in {execution_time:.1f}s verarbeitet"
                )
