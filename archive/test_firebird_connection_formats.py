    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            result = connection.execute(
                text("SELECT rdb$relation_id FROM rdb$database")
            )
            db_id = result.fetchone()
            print(
                f"✅ Connection successful! Database ID: {db_id[0] if db_id else 'N/A'}"
            )
...
def main():
    ...
    if successful_formats:
        ...
        try:
            ...
            print(
                f"   Key Pattern: Use '//' before absolute path for server connections"
            )
        except Exception as e:
