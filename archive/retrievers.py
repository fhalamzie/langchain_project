...
class FaissDocumentationRetriever(BaseDocumentationRetriever):
    ...
    def __init__(
        self, documents: List[Document], embeddings_model: OpenAIEmbeddings, **kwargs
    ):
...
if NEO4J_AVAILABLE:
    ...
    class Neo4jDocumentationRetriever(BaseDocumentationRetriever):
        ...
        def _get_relevant_documents(self, query: str) -> List[Document]:
            ...
            response = self.graph_rag.search(
                query_text=query, retriever_config={"top_k": 5}
            )
            return [
                Document(
                    page_content=response.answer, metadata={"source": "Neo4j GraphRAG"}
                )
            ]
    ...
    class Neo4jDocumentationRetriever:
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "Neo4j is not available. Please install neo4j and neo4j-graphrag packages."
            )
