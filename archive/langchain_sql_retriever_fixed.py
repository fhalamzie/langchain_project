    ...
    def on_chain_end(self, outputs, **kwargs):
        ...
        if self.monitor:
            self.monitor.track_retrieval(
                ...
                * len(self.agent_steps),  # Default relevance scores
                duration=duration,
                success=True,
            )
...
class LangChainSQLRetriever:
    ...
    def _convert_to_server_connection(self, connection_string: str) -> str:
        ...
        """
        try:
            # If already a proper LangChain server connection (firebird://), return as-is
            if (
                "firebird://" in connection_string
                and "localhost:3050" in connection_string
                and connection_string.count("localhost:3050") == 1
            ):
                logger.info(f"🔄 Already LangChain server connection, returning as-is")
            ...
            if "firebird+fdb://" in connection_string:
                ...
                if len(parts) >= 2:
                    ...
                    if "localhost:3050" in db_path:
                        # Remove localhost:3050 and any trailing path separators properly
                        if "localhost:3050//" in db_path:
                            db_path = db_path.replace("localhost:3050//", "/")
                        elif "localhost:3050/" in db_path:
                            db_path = db_path.replace("localhost:3050/", "")
                        else:
                            db_path = db_path.replace("localhost:3050", "")
                    ...
                    server_connection = (
                        f"firebird://{user}:{password}@localhost:3050{db_path}"
                    )
                    logger.info(
                        f"🔄 Converted to server connection: {server_connection}"
                    )
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            langchain_connection = self._convert_to_server_connection(
            ...
            with engine.connect() as conn:
                ...
                logger.info(
                    f"✅ Basic database connection successful. Found ~{table_count} user tables"
                )
            ...
            logger.warning(
                f"Note: LangChain requires Firebird server on localhost:3050"
            )
        ...
        try:
            ...
            if HUB_AVAILABLE:
                try:
                    ...
                    system_message = prompt_template.format(dialect="Firebird", top_k=5)
...
Always examine the table schema before querying. Do NOT make any DML statements."""
            ...
            if LANGGRAPH_AVAILABLE:
                try:
                    ...
                    logger.info(
                        "✅ Enhanced LangGraph ReAct SQL Agent created successfully"
                    )
...
"""
        ...
        try:
            ...
            if LANGGRAPH_AVAILABLE and hasattr(self.agent, "invoke"):
                try:
                    ...
                    result = self.agent.invoke({"messages": [("user", enhanced_query)]})
                    ...
                    logger.warning(
                        f"⚠️ LangGraph invocation failed, trying classic format: {langgraph_error}"
                    )
            ...
            logger.info(
                f"✅ LangChain SQL Agent completed. Generated {len(documents)} documents"
            )
    ...
    def get_retriever_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": (
                "Native LangChain SQL agent with error recovery and schema introspection"
            ),
...
def test_langchain_sql_retriever():
    ...
    try:
        ...
        retriever = LangChainSQLRetriever(
            db_connection_string=db_connection, llm=llm, enable_monitoring=True
        )
