...
class EnhancedWINCASTesting:
    ...
    def run_extensive_test(
        self, concurrent: bool = False, workers: int = 2
    ) -> Dict[str, Any]:
        """Comprehensive testing - extends automated_retrieval_test.py"""
        self.logger.info(
            f"🚀 Starting EXTENSIVE TEST - Comprehensive Mode Analysis (concurrent={concurrent})"
        )
    ...
    def run_baseline_test(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            "🚀 Starting BASELINE TEST - Performance & Statistical Analysis"
        )
    ...
    def _run_concurrent_tests(self, suite: TestSuite, workers: int) -> List[TestResult]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            future_to_task = {
                executor.submit(self.test_single_query, *task): task for task in tasks
            }
            ...
            for future in concurrent.futures.as_completed(future_to_task):
                ...
                try:
                    ...
                    status = "✅" if result.success else "❌"
                    self.logger.info(
                        f"{status} {result.mode}: {result.response_time:.1f}s - {task[0][:50]}..."
                    )
    ...
    def _analyze_mode_performance(self, results: List[TestResult]):
        ...
        for mode in sorted(mode_results.keys()):
            ...
            success_rate = (
                len(successful) / len(results_for_mode) if results_for_mode else 0
            )
            avg_time = (
            ...
            self.logger.info(
                f"   {mode.upper()}: {success_rate:.1%} success, {avg_time:.1f}s avg"
            )
            ...
            if successful:
                ...
                self.logger.info(
                    f"     Fastest: {fastest.response_time:.1f}s - {fastest.query[:30]}..."
                )
                self.logger.info(
                    f"     Slowest: {slowest.response_time:.1f}s - {slowest.query[:30]}..."
                )

    def _generate_summary(
        self, results: List[TestResult], test_type: str
    ) -> Dict[str, Any]:
        ...
        for mode, stats in mode_stats.items():
            self.logger.info(
                f"   {mode}: {stats['success_rate']:.1%} success, {stats['avg_time']:.1f}s avg"
            )
    ...
    def _generate_baseline_summary(self, results: List[TestResult]) -> Dict[str, Any]:
        ...
        if response_times:
            ...
            if len(response_times) >= 10:
                ...
                summary["performance_metrics"]["p90_response_time"] = sorted_times[
                    int(len(sorted_times) * 0.9)
                ]
                summary["performance_metrics"]["p95_response_time"] = sorted_times[
    ...
    def _log_baseline_stats(self, query: str, mode: str, results: List[TestResult]):
        ...
        if times:
            ...
            self.logger.info(
                f"     📈 {avg_time:.1f}s ± {std_time:.1f}s ({success_count}/3 success)"
            )
...
def main():
    ...
    parser.add_argument(
        "--test-type",
        choices=["quick", "extensive", "baseline", "all"],
        default="quick",
    ...
    parser.add_argument(
        "--no-monitoring", action="store_true", help="Disable Phoenix monitoring"
    )
    ...
    if args.test_type != "all":
        ...
        if mode_stats:
            ...
            for i, (mode, stats) in enumerate(sorted_modes, 1):
                print(
                    f"  {i}. {mode}: {stats['success_rate']:.1%} success, {stats['avg_time']:.1f}s avg"
                )
    else:
