...
from global_context import get_global_context_prompt, get_compact_global_context
...
class IterativeImprovementTest:
        ...
        self.test_queries = {
            "basic_lookups": [
                ...
                },
                {
                    ...
                    "expected_filters": ["OPLZORT LIKE '%Köln%'", "EPLZORT LIKE '%Köln%'"],
            ...
            "joins_required": [
                ...
                },
                {
                    ...
                    "expected_joins": ["BEWOHNER.BEWNR = BEWADR.BEWNR", "BEWOHNER.ONR = OBJEKTE.ONR"],
            ...
            "complex_business": [
                {
                    ...
                    "expected_joins": ["EIGENTUEMER.EIGNR = VEREIG.EIGNR", "VEREIG.ONR = OBJEKTE.ONR"],
                ...
                },
                {
                    ...
                    "expected_joins": ["BEWOHNER.KNR = KONTEN.KNR", "KONTEN.KNR = BUCHUNG.KHABEN"],
    ...
    def generate_context_for_version(self, version_key: str, query: str) -> str:
        ...
        if version_config.get("use_retrieval"):
            ...
            context_parts.append("Additional context would be retrieved based on query analysis...")
            context_parts.append("=== END RETRIEVAL ===")
        
        return "\n\n".join(context_parts)
    
    def evaluate_sql_quality(self, sql: str, expected: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if "SELECT" in sql_upper and "FROM" in sql_upper:
            ...
            if not any(error in sql_upper for error in ["SYNTAX ERROR", "INVALID", "ERROR"]):
        ...
        if expected_tables:
            ...
            evaluation["table_selection"] = min(3, (found_tables / len(expected_tables)) * 3)
        else:
            evaluation["table_selection"] = 3  # No specific tables expected
        evaluation["details"].append(f"Table Selection: {evaluation['table_selection']:.1f}/3")
        ...
        if expected_joins:
            ...
            evaluation["details"].append(f"JOIN Logic: {evaluation['join_logic']}/3 (none required)")
        ...
        business_score = 0
        if expected_filters and any(filter_term.split()[0] in sql_upper for filter_term in expected_filters):
            business_score += 1
        ...
        evaluation["details"].append(f"Business Logic: {evaluation['business_logic']}/3")
        ...
        evaluation["details"].append(f"Result Accuracy: {evaluation['result_accuracy']}/3 (estimated)")
    ...
    def run_iteration(self, iteration_number: int, version_key: str) -> Dict[str, Any]:
        ...
        for category, queries in self.test_queries.items():
            ...
            for query_data in queries:
                ...
                print(f"      Score: {evaluation['total_score']}/{evaluation['max_score']} points")
        ...
        all_scores = [r["evaluation"]["total_score"] for r in iteration_result["query_results"]]
        all_max_scores = [r["evaluation"]["max_score"] for r in iteration_result["query_results"]]
        ...
        iteration_result["summary"]["average_score"] = sum(all_scores) / len(all_scores) if all_scores else 0
        iteration_result["summary"]["success_rate"] = (sum(all_scores) / sum(all_max_scores)) if sum(all_max_scores) > 0 else 0
        iteration_result["duration_seconds"] = time.time() - iteration_start
        
        # Identify improvements needed
        low_scores = [r for r in iteration_result["query_results"] if r["evaluation"]["total_score"] < 10]
        for result in low_scores:
            iteration_result["improvements_needed"].append({
                "query": result["query"],
                "score": result["evaluation"]["total_score"],
                "issues": [detail for detail in result["evaluation"]["details"] if "0" in detail or "1" in detail]
            })
        
        # Identify best practices
        high_scores = [r for r in iteration_result["query_results"] if r["evaluation"]["total_score"] >= 12]
        for result in high_scores:
            iteration_result["best_practices_learned"].append({
                "query": result["query"],
                "score": result["evaluation"]["total_score"],
                "sql_pattern": result["generated_sql"][:100] + "..." if len(result["generated_sql"]) > 100 else result["generated_sql"]
            })
        
        print(f"  ✅ Iteration {iteration_number} completed:")
        print(f"     Success Rate: {iteration_result['summary']['success_rate']*100:.1f}%")
        print(f"     Average Score: {iteration_result['summary']['average_score']:.1f}/15")
    ...
    def run_full_test_cycle(self) -> str:
        ...
        print(f"Total queries: {sum(len(queries) for queries in self.test_queries.values())}")
    ...
    def generate_comparison_report(self) -> Path:
        ...
        comparison = {
            "test_metadata": {
                ...
                "total_queries_per_version": sum(len(queries) for queries in self.test_queries.values()),
        ...
        for result in self.iteration_results:
            ...
            for category, scores in result["summary"]["category_scores"].items():
                if scores["average"] >= 12:
                    version_summary["strengths"].append(f"Strong in {category} (avg: {scores['average']:.1f})")
                elif scores["average"] < 8:
                    version_summary["weaknesses"].append(f"Weak in {category} (avg: {scores['average']:.1f})")
            
            comparison["version_comparisons"].append(version_summary)
        
        # Find best version
        best_result = max(self.iteration_results, key=lambda x: x["summary"]["success_rate"])
        comparison["best_version"] = {
            "version": best_result["version"],
            "success_rate": best_result["summary"]["success_rate"],
            "reason": f"Highest success rate of {best_result['summary']['success_rate']*100:.1f}%"
        }
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if common_issues:
            ...
            recommendations.append(f"Address recurring issue: {top_issue[0]} (appears in {top_issue[1]} test cases)")
        
        # Performance recommendations
        avg_success_rate = sum(r["summary"]["success_rate"] for r in self.iteration_results) / len(self.iteration_results)
        if avg_success_rate < 0.75:
            recommendations.append("Overall success rate is below 75% - focus on improving basic query understanding")
        
        # Context optimization
        best_version = max(self.iteration_results, key=lambda x: x["summary"]["success_rate"])
        recommendations.append(f"Use '{best_version['version']}' as base configuration ({best_version['summary']['success_rate']*100:.1f}% success rate)")
        
        if len(recommendations) == 0:
            recommendations.append("All versions performing well - consider testing more complex scenarios")
    ...
    def create_readable_summary(self, comparison: Dict[str, Any], summary_file: Path):
        ...
        lines.append(f"- Versions tested: {comparison['test_metadata']['total_versions_tested']}")
        lines.append(f"- Queries per version: {comparison['test_metadata']['total_queries_per_version']}")
        lines.append(f"- Categories: {', '.join(comparison['test_metadata']['query_categories'])}")
        lines.append("")
