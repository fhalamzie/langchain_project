...
try:
    ...
    for path in fb_lib_paths:
        if path.exists():
            ...
            if path.is_dir():
                lib_files = list(path.glob("**/libfbclient.so*")) + list(path.glob("**/fbclient.dll"))
                if lib_files:
                    fb_lib_path = lib_files[0]
                    fb_lib_found = True
                    logger.info(f"Firebird-Client-Library im Verzeichnis gefunden: {fb_lib_path}")
                    break
    
    # Ausgabe des Suchpfads
    logger.info(f"Suche Firebird-Client-Library in: {', '.join(str(p) for p in fb_lib_paths)}")
    ...
    logger.error("Firebird-Driver nicht gefunden. Installiere ihn mit: pip install firebird-driver")
...
def get_db_connection():
    ...
    try:
        if not FIREBIRD_AVAILABLE:
            raise ImportError("Firebird-Driver nicht verfügbar. Installiere ihn mit: pip install firebird-driver")
        ...
        if "location of Firebird Client Library" in str(e):
            ...
            logger.error("Überprüfen Sie, ob die Library-Datei im korrekten Pfad existiert.")
        elif "Permission denied" in str(e):
            logger.error("Zugriffsverweigerung beim Zugriff auf Dateien oder Verzeichnisse.")
            logger.error("Überprüfen Sie die Berechtigungen für die Datenbank-Datei und temporäre Verzeichnisse.")
