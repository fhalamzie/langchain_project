...
def sample_yaml_docs():
    ...
    return [
        {
            "page_content": "BEWOHNER table contains resident information with fields: BEWNR, BVNAME, BNAME, ONR",
            "metadata": {"source": "BEWOHNER.yaml", "table": "BEWOHNER"}
        },
        {
            "page_content": "OBJEKTE table contains property information with primary key ONR",
    ...
    def assert_sql_contains_joins(self, sql: str, expected_joins: List[str]):
        ...
        for join in expected_joins:
            ...
            if "=" in join:
                ...
                assert left.strip().upper() in sql_upper and right.strip().upper() in sql_upper
    
    def assert_response_time_acceptable(self, execution_time: float, max_time: float = 30.0):
        """Assert that response time is within acceptable limits"""
        assert execution_time <= max_time, f"Response time {execution_time:.2f}s exceeds {max_time}s"
...
class TestBusinessGlossar(FeatureTestBase):
    ...
    @pytest.mark.parametrize("term,expected_table", [
        ("Kredit", "SOLLSTELLUNG"),
        ("Mieter", "BEWOHNER"), 
...
class TestSQLValidator(FeatureTestBase):
    ...
    @pytest.mark.parametrize("invalid_sql,expected_issue", [
